...
class FFTMultiClassSVI(BaseTask, BaseInference):
    """
    Bayesian Multiclass Classification Model with FFT-based Circulant Matrix Layer using SVI inference.
    """
    ...
    def bayesian_inference(self, X_train, y_train, rng_key):
        ...
        svi_state = self.svi.init(
            rng_key, X_train, y_train, num_classes=self.num_classes
        )
    ...
    def predict(self, X_test, rng_key, num_samples=100):
        ...
        pred_samples = predictive(rng_key, X=X_test, num_classes=self.num_classes)
...
if __name__ == "__main__":
    ...
    ############# Example Usage #############
    from BNN.modules.fft.multiclass_classification.svi import FFTMultiClassSVI
    ...
    key = jr.key(0)
    classifier = FFTMultiClassSVI(
        num_steps=1000, model_type="deep", num_classes=3, track_loss=True
    )
