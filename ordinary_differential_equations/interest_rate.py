# Full Script for Vasicek Interest Rate Model
import functools
...
def dz_dt(z, t, theta):
    r = z[0]  # Interest rate
    a, b, sigma = theta  # Mean-reversion speed, long-term mean, and volatility
    dr_dt = a * (b - r)  # Deterministic part of the Vasicek model
...
def model(ts, y_init, y=None):
    ...
    theta = numpyro.sample(
        ...
        dist.TruncatedNormal(
            low=0.0,
            loc=jnp.array([0.2, 0.05, 0.02]),
            scale=jnp.array([0.1, 0.02, 0.01]),
        ),
    )
...
def generate_synthetic_data(key, n_timesteps=100):
    ...
    true_params = jnp.array([0.2, 0.05, 0.02])  # a, b, sigma
    y_init = jnp.array([0.03])  # Initial interest rate
...
def main():
    ...
    plot_results(
        ts, y, odeint(dz_dt, y_init, ts, jnp.array([0.2, 0.05, 0.02])), predictions
    )
