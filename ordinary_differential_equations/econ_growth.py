# Full Script for Economic Growth (Solow Model)
import functools
...
# Solow Growth Model
def dz_dt(z, t, theta):
    k = z[0]  # Capital per worker
    s, delta, n = theta
    f_k = k**0.5  # Cobb-Douglas production function
    dk_dt = s * f_k - (delta + n) * k
    return jnp.array([dk_dt])
...
def model(ts, y_init, y=None):
    ...
    theta = numpyro.sample(
        ...
        dist.TruncatedNormal(
            low=0.0, loc=jnp.array([0.3, 0.1, 0.02]), scale=jnp.array([0.1, 0.05, 0.01])
        ),
    )
...
def generate_synthetic_data(key, n_timesteps=100):
    ...
    true_params = jnp.array([0.3, 0.1, 0.02])  # s, delta, n
    y_init = jnp.array([1.0])  # Initial capital per worker
...
def main():
    ...
    plot_results(
        ts, y, odeint(dz_dt, y_init, ts, jnp.array([0.3, 0.1, 0.02])), predictions
    )
