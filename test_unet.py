...
# If not already done:
# from your_bnn_library import Module, Conv2d, TransposedConv2d, MaxPool2d
...
"""# Suppose we have a batch of grayscale images:
rng_key = random.PRNGKey(0)
X = random.normal(rng_key, shape=(4, 1, 128, 128))  # 4 images, 1 channel, 128x128
y = random.bernoulli(rng_key, p=0.3, shape=(4, 1, 128, 128))  # binary masks

unet = UNet(in_channels=1, out_channels=1, method="svi", task_type="binary")
unet.compile(num_warmup=50, num_samples=100, num_chains=1)
unet.fit(X, y, rng_key, num_steps=10)
preds = unet.predict(X, rng_key, posterior="logits")
...
# Example usage of FFTUNet:
rng_key = jax.random.PRNGKey(42)
X = jax.random.normal(rng_key, shape=(4, 1, 128, 128))  # 4 images, 1 channel, 128x128
y = jax.random.bernoulli(rng_key, p=0.5, shape=(4, 1, 128, 128))

fft_unet = FFTUNet(in_channels=1, out_channels=1, method="nuts", task_type="binary")
fft_unet.compile(num_warmup=50, num_samples=100, num_chains=1)
fft_unet.fit(X, y, rng_key)
