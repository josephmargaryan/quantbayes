...
class GaussianMixtureModel:
    ...
    def initialize(self, data, seed):
        init_values = {
            ...
            "locs": data[
                random.categorical(
                    random.PRNGKey(seed), jnp.ones(len(data)) / len(data), shape=(self.num_components,)
                )
            ],
        }
        ...
        global_model = handlers.block(
            ...
            hide_fn=lambda site: site["name"] not in ["weights", "scale", "locs", "components"],
        )
    ...
    def classify(self, data, temperature=0, rng_key=None):
        inferred_model = infer_discrete(
            ...
            temperature=temperature, first_available_dim=-2, rng_key=rng_key
        )
    ...
    def _get_guide_trace(self, data):
        trained_global_guide = handlers.substitute(self.global_guide, self.trained_params)
    ...
    def plot_results(self, data):
        ...
        Y = sum(weights[k] * scipy.stats.norm.pdf((X - locs[k]) / scale) for k in range(self.num_components))
