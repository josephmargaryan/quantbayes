...
class VisionTransformerDiffusion(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=8, in_channels=3,
                 embed_dim=256, num_heads=8, num_layers=4, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x, t):
        ...
        x = x.view(B, h_patches, w_patches, self.patch_size, self.patch_size, self.in_channels)
...
class TimeSeriesTransformer(nn.Module):
    ...
    """
    def __init__(self, seq_len=100, input_dim=1, embed_dim=64,
...
class TabularDiffusionModel(nn.Module):
    ...
    def __init__(self, 
        ...
        for i in range(num_layers):
            ...
            prev_dim = hidden_dim
        layers.append(nn.Linear(hidden_dim, input_dim))  # final output matches input dimension
    ...
    def forward(self, cont_x, cat_x, t):
        ...
        if self.categorical_dims is not None and cat_x is not None:
            ...
            # Concatenate
            cat_emb_concat = torch.cat(cat_emb_list, dim=-1)  # shape (B, sum_of_embed_dims)
            x = torch.cat([cont_x, cat_emb_concat], dim=-1)   # final shape (B, input_dim)
        else:
            # Only continuous
            x = cont_x
