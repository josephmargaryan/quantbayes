...
class BetaSchedule:
    def __init__(self, schedule_type='linear', timesteps=1000, beta_start=1e-4, beta_end=2e-2, **kwargs):
...
class GaussianDiffusion(nn.Module):
    def __init__(self, model, beta_schedule: BetaSchedule, sampling_timesteps=None, use_ddim=False):
        """
    ...
    def forward_diffusion_sample(self, x0, t, device='cuda'):
        ...
        alphas_cumprod_t = self.alphas_cumprod[t].reshape(batch_size, *((1,) * (len(x0.shape) - 1)))
        noise = torch.randn_like(x0, device=device)
        return torch.sqrt(alphas_cumprod_t) * x0 + torch.sqrt(1 - alphas_cumprod_t) * noise, noise
