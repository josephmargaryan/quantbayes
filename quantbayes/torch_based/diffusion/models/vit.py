...
def test_dit_forward():
    ...
    input_size = 32           # Spatial dimensions of the input image
    patch_size = 2
    in_channels = 4
    hidden_size = 1152
    depth = 2                 # Reduced depth for faster testing (default is 28)
    num_heads = 16
    mlp_ratio = 4.0
    class_dropout_prob = 0.1
    num_classes = 1000
    learn_sigma = True        # Determines output channels: if True, out_channels = in_channels * 2
    ...
    # For learn_sigma=True, out_channels = in_channels * 2
    out_channels = in_channels * 2 if learn_sigma else in_channels
    # The unpatchify function should return images of the same spatial dimensions as the input.
    expected_shape = (batch_size, out_channels, input_size, input_size)
    
    assert output.shape == expected_shape, f"Expected shape {expected_shape}, got {output.shape}"
    print("DiT forward test passed. Output shape:", output.shape)
