...
from quantbayes.torch_based.vision.pipeline.segmentation_pipeline import SegmentationPipeline
from quantbayes.torch_based.vision.pipeline.classification_pipeline import ClassificationPipeline
...
def test_segmentation_pipeline():
    ...
    assert isinstance(pred, np.ndarray) and pred.shape[:2]==sample.shape[:2], "Pred shape OK"
...
def test_classification_pipeline():
    ...
    pipe.fit(tr_ds, vl_ds, epochs=1, batch_size=4, save_dir=os.path.join(tmp,"ckpt_cls"))
