...
class Trainer:
    ...
    def __init__(
        ...
        if self.cfg.compile:
            try:
                ...
                self.logger.warning("torch.compile() failed (%s); proceeding without.", str(e))
        ...
        self.amp_dtype = torch.bfloat16 if self.cfg.amp_dtype.lower() == "bf16" else torch.float16
    ...
    def train(
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            val_logs = self._run_epoch(val_loader, training=False) if val_loader is not None else {}
    ...
    def _resolve_monitor_value(self, train_logs: Dict[str, float], val_logs: Dict[str, float]) -> float:
    ...
    def _extract_xy(self, batch) -> Tuple[Any, Any]:
        ...
        if isinstance(batch, dict):
            ...
            if x is not None and y is not None:
                return x, y
        raise ValueError("Batch format not understood. Expected (x, y) or dict with inputs/targets.")

    def _compute_metrics(self, preds: torch.Tensor, y: torch.Tensor) -> Dict[str, float]:
        out: Dict[str, float] = {}
        for name, fn in self.metrics.items():
            try:
                val = fn(preds, y)
                out[name] = float(val) if isinstance(val, (int, float)) else float(val.detach().cpu().item())
            except Exception as e:
        ...
        for old in self.best_checkpoints[self.cfg.save_best_k:]:
            if os.path.exists(old):
                try:
                    ...
                    self.logger.warning("Failed removing old checkpoint %s: %s", old, str(e))
        self.best_checkpoints = self.best_checkpoints[: self.cfg.save_best_k]

    # --------------------------- Epoch Runner --------------------------- #

    def _run_epoch(self, loader: Optional[DataLoader], training: bool) -> Dict[str, float]:
        if loader is None:
        ...
        for batch_idx, batch in enumerate(loader):
            try:
                ...
                # Non-finite loss handling
                if not (loss_value == loss_value and (loss_value != float("inf") and loss_value != -float("inf"))):  # NaN/Inf check
                    if self.cfg.nonfinite_skip:
                        self.logger.warning("Non-finite loss at batch %d; skipping.", batch_idx)
                        if training:
                            self.optimizer.zero_grad(set_to_none=self.cfg.zero_grad_set_to_none)
                        continue
                    else:
                ...
                if training:
                    ...
                    if step_boundary:
                        ...
                        if self.scheduler and self.cfg.scheduler_step == "batch" and not isinstance(self.scheduler, ReduceLROnPlateau):
                            self.scheduler.step()
                ...
                with torch.no_grad():
                    batch_metrics = self._compute_metrics(preds, y) if self.metrics else {}
                ...
                # Periodic logging
                if training and self.cfg.log_interval and (batch_idx + 1) % self.cfg.log_interval == 0:
                    self.logger.info(
                        "Epoch %d | batch %d | loss=%.4f",
                        self.state["epoch"] + 1, batch_idx + 1, loss_value
                    )
        ...
        if training and seen_batches > 0 and (seen_batches % self.cfg.grad_accum_steps != 0):
            if self.cfg.max_grad_norm is not None:
                if self.scaler:
                    self.scaler.unscale_(self.optimizer)
                nn.utils.clip_grad_norm_(self.model.parameters(), self.cfg.max_grad_norm)
        ...
        avg_loss = (epoch_loss_sum / max(1, seen_batches)) if seen_batches > 0 else float("inf")
        ...
        if not training:
            ...
            logs = { (f"val_{k}" if not k.startswith("val_") else k) : v for k, v in logs.items() }
...
if __name__ == "__main__":
    ...
    print("Best checkpoint saved at:", trainer.best_checkpoints[0] if trainer.best_checkpoints else "None")
