...
from quantbayes.torch_based.trainer.trainer import Trainer, TrainerConfig  # adjust import as needed
...
class AugmentedDataset(Dataset):
    def __init__(self, X: torch.Tensor, y: torch.Tensor,
                 augment_fn: Optional[Callable[[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor]]] = None):
        self.X = X
        self.y = y
...
class TorchRegressor(_TorchBase, RegressorMixin):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        X_tr, X_va, y_tr, y_va = train_test_split(
            X_t, y_t, test_size=self.val_frac,
            shuffle=True, random_state=self.trainer_kwargs.get("seed",42)
        )
...
class TorchBinaryClassifier(_TorchBase, ClassifierMixin):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        self.trainer = Trainer(self.model,optim,loss_fn,sched,
...
class SegmentationEnsemble(BaseEstimator):
        ...
        if self.ensemble_method == "stacking":
            ...
            for i, name in enumerate(names):
                ...
                for tr, va in self.cv.split(X, y.reshape(N, -1)[:,0]):
                    inst = clone(m)
                    inst.fit(X[tr], y[tr])
                    probs = inst.predict_proba(X[va])[:,1,...]  # single foreground channel
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        probs = [self.fitted_[n].predict_proba(X) for n in names]  # each (N,2,H,W) or (N,C,H,W)
        # if binary, shape is (N,2,H,W) -> take foreground channel
        if probs[0].shape[1] == 2:
            probs = [p[:,1,...] for p in probs]
        stacked = np.stack(probs, axis=-1)  # (N,H,W,M)
        if self.ensemble_method == "average":
            w = np.array([self.weights.get(n,1.0) for n in names]) if self.weights else np.ones(len(names))
            w /= w.sum()
...
if __name__ == "__main__":
    ...
    seg1 = TorchImageSegmenter(ToySegModel, {"in_channels":C,"classes":2},
                               {"device":"cpu"}, batch_size=8, num_epochs=5,
                               patience=2, augment_fn=random_flip)
    seg2 = TorchImageSegmenter(ToySegModel, {"in_channels":C,"classes":2},
                               {"device":"cpu"}, batch_size=8, num_epochs=5,
                               patience=2, augment_fn=random_flip)

    ensemble = SegmentationEnsemble(
        models={"s1":seg1,"s2":seg2},
