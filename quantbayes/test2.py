...
def run_sde(
    ...
    # Limit the number of trajectories visualized in the plot
    num_plot_trajectories = min(ts.shape[0], max_plot_trajectories)
    ts_limited = ts[:num_plot_trajectories]
    ys_limited = ys[:num_plot_trajectories]
    ys_generated_limited = ys_generated[:num_plot_trajectories]

    # Plot
    fig, ax = plt.subplots()
    for idx in range(ts_limited.shape[0]):
        ax.plot(
            ts_limited[idx],
            ys_limited[idx, :, 0],
            c="blue",
    ...
    for idx in range(ts_limited.shape[0]):
        ax.plot(
            ts_limited[idx],
            ys_generated_limited[idx],
            c="red",
            alpha=0.7,
            label="Generated" if idx == 0 else None,
        )
    ax.set_title(f"Original and Generated Trajectories (max {max_plot_trajectories})")
    ax.set_xlabel("Time")
