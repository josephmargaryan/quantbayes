...
class Mamba(nn.Module):
    ...
    def __call__(self, x, carry: Optional[jnp.ndarray] = None):
        ...
        for t in range(seq_len):
            ...
            zx = jnp.concatenate([z, x_t], axis=-1)  # Shape: (batch_size, hidden_dim + input_dim)
            # Update hidden state
...
def test_mamba_state_space():
    ...
    x = jax.random.normal(key, (batch_size, seq_len, input_dim))  # (batch=8, seq_len=20, input_dim=5)
