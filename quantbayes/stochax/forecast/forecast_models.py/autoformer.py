...
class Autoformer(eqx.Module):
    ...
    def __call__(self, x, *, key=None):
        ...
        def process_sample(x_sample, key_sample):
            keys = jax.random.split(key_sample, len(self.layers)) if key_sample is not None else [None]*len(self.layers)
...
if __name__ == '__main__':
    ...
    model = Autoformer(num_layers=3, embed_dim=D, num_heads=4, dropout_p=0.1, key=model_key)
