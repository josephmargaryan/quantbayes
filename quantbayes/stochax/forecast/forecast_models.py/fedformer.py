...
class Fedformer(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.trend_net = MLP(
            embed_dim, int(embed_dim * mlp_ratio), dropout_p, key=key_trend
        )
    ...
    def __call__(self, x: jnp.ndarray, *, key=None) -> jnp.ndarray:
        ...
        if key_trend is not None:
            ...
            keys_trend = None
        trend_out = jax.vmap(self.trend_net)(
            trend, key=keys_trend
        )  # shape: [seq_len, embed_dim]
...
if __name__ == "__main__":
    ...
    model = FedformerForecast(
        embed_dim=D, num_heads=4, dropout_p=0.1, kernel_size=3, key=model_key
    )
