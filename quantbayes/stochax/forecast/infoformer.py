...
class ProbMask:
    ...
    def __init__(self, B: int, H: int, L: int, index: jnp.ndarray, scores: jnp.ndarray):
        ...
        _mask_ex = jnp.broadcast_to(
            _mask[None, None, :, :], (B, H, L, scores.shape[-1])
        )
...
class FullAttention(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        scores = (
            jnp.einsum("bhqd,bhkd->bhqk", queries, keys) * scale
        )  # [B, H, L_Q, L_K]
...
class ProbAttention(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        context, attn = self._update_context(
            context, values, scores_top, index, L_Q, attn_mask, B, H, rngs, train
        )
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):
        ...
        # Compute Q * K_sample
        Q_K_sample = jnp.einsum(
            "bhqd,bhqkd->bhqk", Q, K_sample
        )  # [B, H, L_Q, sample_k]
    ...
    def _get_initial_context(self, V, L_Q):
        if not self.mask_flag:
            context = jnp.mean(V, axis=2, keepdims=True).repeat(
                L_Q, axis=2
            )  # [B, H, L_Q, D]
        else:
    ...
    def _update_context(
        self, context_in, V, scores, index, L_Q, attn_mask, B, H, rngs, train
    ):
        if self.mask_flag and attn_mask is not None:
        ...
        updated_context = context_in.at[:, :, index, :].set(
        ...
        if self.output_attention:
            ...
            full_attn = jnp.zeros(
                (B, H, context_in.shape[2], V.shape[2]), dtype=attn.dtype
            )
...
class AttentionLayer(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        query_proj = nn.Dense(
            features=H * d_keys, use_bias=False, name="query_projection"
        )
        key_proj = nn.Dense(features=H * d_keys, use_bias=False, name="key_projection")
        value_proj = nn.Dense(
            features=H * d_values, use_bias=False, name="value_projection"
        )
        out_proj = nn.Dense(
            features=self.d_model, use_bias=False, name="out_projection"
        )
...
class ConvLayer(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        # 1. Circular Padding
        x_padded = jnp.pad(
            x, pad_width=((0, 0), (1, 1), (0, 0)), mode="wrap"
        )  # [B, L+2, D]
        ...
        x_pooled = x_trimmed.reshape(B, L_new // 2, 2, D).max(
            axis=2
        )  # [B, L_new//2, D]

        # 6. Dropout
        x_drop = nn.Dropout(rate=self.dropout)(
            x_pooled,
            deterministic=not train,
            rng=rngs["dropout"] if rngs and "dropout" in rngs else None,
        )  # [B, L_new//2, D]
...
class PositionalEncoding(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        assert pos_encoding.shape == (
            ...
            seq_len,
            self.d_model,
        ), f"Positional encoding shape mismatch: {pos_encoding.shape}"
        assert (
            x.shape[2] == self.d_model
        ), f"Feature dimension mismatch: {x.shape[2]} vs {self.d_model}"
...
class DataEmbedding(nn.Module):
    ...
    def __call__(
        ...
        assert (
            value_emb.shape[2] == self.d_model
    ...
    def __call__(
        ...
        *,
        ...
        new_x, attn = self.attention(
            x, x, x, attn_mask=attn_mask, train=train, rngs=rngs
        )
        ...
        assert (
            new_x.shape == x.shape
        ), f"Shape mismatch before residual addition: {new_x.shape} vs {x.shape}"
        ...
        y_ffn = nn.Dense(features=self.d_model, use_bias=True, name="conv2")(
            y_ffn
        )  # [B, L, d_model]

        # Dropout
        y_ffn = nn.Dropout(rate=self.dropout)(
            y_ffn,
            deterministic=not train,
            rng=rngs["dropout"] if rngs and "dropout" in rngs else None,
        )  # [B, L, d_model]
        ...
        assert (
            y_ffn.shape == x.shape
        ), f"Shape mismatch before second residual addition: {y_ffn.shape} vs {x.shape}"
        ...
        *,
        ...
        if self.conv_layers is not None:
            ...
            for attn_layer, conv_layer in zip(self.attn_layers[:-1], self.conv_layers):
                ...
                assert (
                    x.ndim == 3
                ), f"ConvLayer output has incorrect number of dimensions: {x.shape}"
                assert (
                    x.shape[2] == self.attn_layers[0].d_model
                ), f"ConvLayer feature dimension mismatch: {x.shape[2]} vs {self.attn_layers[0].d_model}"
            # Last layer
            x, attn = self.attn_layers[-1](
                x, attn_mask=attn_mask, train=train, rngs=rngs
            )
            attns.append(attn)
        else:
            # No distilling
        ...
        if self.norm_layer is not None:
            ...
            assert (
                x.ndim == 3
            ), f"Normalization output has incorrect number of dimensions: {x.shape}"
...
class Informer(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        """
        # 1. Embedding
        enc_out = DataEmbedding(
            c_in=self.input_dim, d_model=self.d_model, dropout=self.dropout
        )(
        ...
        assert (
            enc_out.ndim == 3
        ), f"Embedding output has incorrect number of dimensions: {enc_out.shape}"
        assert (
            enc_out.shape[2] == self.d_model
        ), f"Embedding feature dimension mismatch: {enc_out.shape[2]} vs {self.d_model}"
        ...
        # 4. Encoder forward
        enc_out, attns = encoder(
            enc_out, attn_mask=None, train=train, rngs=rngs
        )  # [B, L', d_model]

        # Debugging: Check shape after encoder
        assert (
            enc_out.ndim == 3
        ), f"Encoder output has incorrect number of dimensions: {enc_out.shape}"
        assert (
            enc_out.shape[2] == self.d_model
        ), f"Encoder feature dimension mismatch: {enc_out.shape[2]} vs {self.d_model}"
        ...
        assert (
            last_hidden.ndim == 2
        ), f"Last hidden state has incorrect number of dimensions: {last_hidden.shape}"
        assert (
            last_hidden.shape[1] == self.d_model
        ), f"Last hidden state feature dimension mismatch: {last_hidden.shape[1]} vs {self.d_model}"
...
def test_informer():
    ...
    variables = informer.init(
        {"params": init_rng, "dropout": dropout_rng}, x, train=True
    )
