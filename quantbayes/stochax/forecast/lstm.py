    ...
    def __call__(self, x, carry=None):
        ...
        if carry is None:
            ...
            for _ in range(self.num_layers):
                ...
                carry.append(lstm_cell.initialize_carry(jax.random.PRNGKey(0), (batch_size, self.hidden_dim)))
            carry = tuple(carry)

        for layer in range(self.num_layers):
            # Pass hidden_dim to LSTMCell
            lstm_cell = nn.LSTMCell(features=self.hidden_dim, name=f'2lstm_cell_{layer}')
            hidden = carry[layer]
            outputs = []
