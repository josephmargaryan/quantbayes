...
def _alpha_bounded(
    ...
    """Clamp α to [lo, hi] via a sigmoid map (keeps gradients healthy)."""
    return lo + (hi - lo) * jax.nn.sigmoid(alpha_raw)


class SpectralCirculantLayer(eqx.Module, _SpectralMixin):
    """
    1D spectral-circulant linear map with trainable smoothness exponent α.
    FFT/IFFT are unitary ("ortho"). Supports cropping back to in_features.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.w_real = w_r
        self.w_imag = w_i
        self.bias = jr.normal(k3, (self.padded_dim,), dtype) * bias_scale
...
class AdaptiveSpectralCirculantLayer(eqx.Module, _SpectralMixin):
    ...
    bounded total exponent via sigmoid map. Exposes `delta_alpha` for your loop.
    """
    ...
    def delta_alpha(self) -> jnp.ndarray:
        """For your global penalty: softplus(delta_z)."""
    ...
    def _spectral_scale(self) -> jnp.ndarray:
        δ = self.delta_alpha
        α_total = _alpha_bounded(self.alpha_raw + δ)  # re-clamp
        k = jnp.arange(self.k_half, dtype=self.w_real.dtype)
        k_norm = k / jnp.maximum(self.k_half - 1, 1)
...
class SpectralCirculantLayer2d(eqx.Module, _SpectralMixin):
    ...
    - K_rad in [0,1]: keep modes with ||ω||/||ω||_max <= K_rad
      (set None to keep all). Use soft mask if desired.
    - crop_output: crop spatial output back to (H_in, W_in).
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.w_imag = wi
        self.bias = (
            jr.normal(k3, (self.C_out, self.H_pad, self.W_pad), dtype) * bias_scale
        )
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        y = y + (self.bias if y.ndim == 3 else self.bias[None, ...])
        if not self.crop_output or (
            self.H_pad == self.H_in and self.W_pad == self.W_in
        ):
            return y
        return y[..., : self.H_in, : self.W_in]
...
class AdaptiveSpectralCirculantLayer2d(SpectralCirculantLayer2d):
    ...
    Exposes `delta_alpha` for your global penalty.
    """
    ...
    def _spectral_scale(self) -> jnp.ndarray:
        ...
        R = jnp.sqrt(U**2 + V**2)
        R_norm = R / jnp.maximum(jnp.max(R), 1.0)
        α_total = _alpha_bounded(self.alpha_raw + self.delta_alpha)  # re-clamp
...
class SpectralDense(eqx.Module, _SpectralMixin):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        # x: (..., in) -> (..., out)
        W = self.U @ (self.s * self.V.T)  # (out, in)
...
class SpectralConv2d(eqx.Module, _SpectralMixin):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        W_mat = self.U @ (self.s * self.V.T)  # (C_out, C_in*H_k*W_k)
...
class RFFTCirculant1D(eqx.Module, _SpectralMixin):
    """
    1D spectral-circulant linear map with RFFT storage (half-spectrum).
      y = irfft( rfft(x_pad) * H_half ), norm='ortho'
    - Stores complex H_half ∈ C^{K_half}
    - K: keep first K frequency bins (hard truncation), else keep all K_half
    - crop_output: crop back to in_features if padded_dim>in_features
    """
    ...
    bias: jnp.ndarray  # (padded_dim,) real
    ...
    def __init__(
        ...
        *,
        ...
        # init with Sobolev-shaped std across bins
        k = jnp.arange(self.k_half, dtype=dtype)
        k_norm = k / jnp.maximum(self.k_half - 1, 1)
        α0 = _alpha_bounded(self.alpha_raw)
        std0 = 1.0 / jnp.sqrt(1.0 + k_norm**α0)
        ...
        self.H_half = w_r + 1j * w_i
        self.bias = jr.normal(k1, (self.padded_dim,), dtype) * bias_scale
    ...
    def _masked_half(self) -> jnp.ndarray:
        if self.K >= self.k_half:
            return self.H_half
        mask = (jnp.arange(self.k_half) < self.K).astype(self.H_half.real.dtype)
        return (self.H_half.real * mask) + 1j * (self.H_half.imag * mask)

    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        # pad/crop to padded_dim on the last axis
        d_in = x.shape[-1]
        ...
        Xf = jnp.fft.rfft(x_pad, axis=-1, norm="ortho")  # (..., k_half)
        Yf = Xf * self._masked_half()
...
class RFFTCirculant2D(eqx.Module, _SpectralMixin):
    """
    2D spectral-circulant conv with RFFT2 storage:
      y = irfft2( rfft2(x_pad) * K_half ), norm='ortho'
    - K_half: (C_out, C_in, H_pad, W_half) complex
    - Optional radial low-mode mask K_rad ∈ [0,1] (hard or soft)
    - crop_output=True crops back to (H_in, W_in)
    """
    ...
    bias: jnp.ndarray  # (C_out, H_pad, W_pad) real
    ...
    def __init__(
        ...
        *,
        ...
        # normalized radial grid aligned with RFFT2 layout (H_pad, W_half)
        u = jnp.fft.fftfreq(self.H_pad, dtype=dtype) * self.H_pad  # H_pad values
        v = jnp.fft.rfftfreq(self.W_pad, dtype=dtype) * self.W_pad  # W_half values
        U, V = jnp.meshgrid(u, v, indexing="ij")
        R = jnp.sqrt(U**2 + V**2)
        R_norm = R / jnp.maximum(jnp.max(R), 1.0)

        α0 = _alpha_bounded(self.alpha_raw)
        std0 = 1.0 / jnp.sqrt(1.0 + R_norm**α0)  # (H_pad, W_half)
        ...
        # Imag must be zero on columns v=0 and (if even) v=W/2
        wi = wi.at[..., :, 0].set(0.0)
        if (self.W_pad % 2 == 0) and (self.W_half > 1):
            wi = wi.at[..., :, -1].set(0.0)

        self.K_half = wr + 1j * wi
        self.bias = (
            jr.normal(k3, (self.C_out, self.H_pad, self.W_pad), dtype) * bias_scale
        )
    ...
    def _spectral_weights(self) -> jnp.ndarray:
        return jnp.concatenate(
            [self.K_half.real.reshape(-1), self.K_half.imag.reshape(-1)]
        )

    def _spectral_scale(self) -> jnp.ndarray:
        u = jnp.fft.fftfreq(self.H_pad, dtype=self.K_half.real.dtype) * self.H_pad
        v = jnp.fft.rfftfreq(self.W_pad, dtype=self.K_half.real.dtype) * self.W_pad
        U, V = jnp.meshgrid(u, v, indexing="ij")
        R = jnp.sqrt(U**2 + V**2)
        R_norm = R / jnp.maximum(jnp.max(R), 1.0)
        α = _alpha_bounded(self.alpha_raw)
        β2d = 1.0 + R_norm**α  # (H_pad, W_half)
        flat = jnp.broadcast_to(β2d, self.K_half.real.shape[2:]).reshape(-1)
        # replicate per (C_out, C_in)
        flat = jnp.tile(flat, self.C_out * self.C_in)
        return jnp.concatenate([flat, flat])

    # ----- low-pass mask in half-plane -----
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        # Yf[b, o, h, w] = sum_i K[o,i,h,w] * Xf[b,i,h,w]
...
class SpectralTokenMixer(eqx.Module, _SpectralMixin):
    ...
    Spectral Token Mixer for sequences:
      x ∈ R^{B,N,C} → y = iRFFT( RFFT(x along N) ⊙ H_half ) with channel groups.
    - N is fixed at init (like MLP-Mixer/ViT patch count).
    - Grouped: share one spectral filter per group of channels (C/G groups).
    - Residual-friendly: returns x + mix(x) if use_residual=True.

    Penalty uses β(k) = 1 + k_norm^α with learnable bounded α.

    Args:
      n_tokens: sequence length N.
      channels: channel dimension C.
      groups: number of channel groups (divides C). groups=1 → shared; groups=C → per-channel.
      alpha_init: initial smoothness exponent.
    ...
    def __init__(
        ...
        *,
        ...
        # lightweight channel gate (like Mixer MLP scaling)
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        # x: (B, N, C)
        B, N, C = x.shape
        assert N == self.n_tokens and C == self.channels
        ...
        # For each group g, channels [g*ch_per_g:(g+1)*ch_per_g] share H[g]
        def apply_group(g, Xf):
            sl = slice(g * ch_per_g, (g + 1) * ch_per_g)
            return Xf[:, :, sl] * H[g][None, :, None]

        Xf_out = []
        for g in range(G):
            Xf_out.append(apply_group(g, Xf))
        Xf_out = jnp.concatenate(Xf_out, axis=-1)
...
class SVDDense(eqx.Module, _SpectralMixin):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        # y = U diag(s) V^T x
        z = x @ self.V  # (..., r)
        z = z * self.s  # broadcast
        y = z @ self.U.T  # (..., out)
        return y + self.bias


# graph_cheb_sobolev.py
# Copyright:
#   MIT-style license. Production-ready Equinox module.
#
# Summary
# -------
