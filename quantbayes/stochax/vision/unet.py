...
class UNet(nn.Module):
    ...
    def __call__(self, x):
        ...
        conv3 = nn.Conv(
            features=256, kernel_size=(3, 3), strides=(1, 1), padding="SAME"
        )(pool2)
        conv3 = nn.relu(conv3)
        conv3 = nn.Conv(
            features=256, kernel_size=(3, 3), strides=(1, 1), padding="SAME"
        ...
        # Bottleneck
        bottleneck = nn.Conv(
            features=512, kernel_size=(3, 3), strides=(1, 1), padding="SAME"
        )(pool3)
        ...
        # Upsample path
        up3 = nn.ConvTranspose(features=256, kernel_size=(2, 2), strides=(2, 2))(
            bottleneck
        )
        ...
        up2 = nn.ConvTranspose(features=128, kernel_size=(2, 2), strides=(2, 2))(
            conv_up3
        )
        ...
        up1 = nn.ConvTranspose(features=64, kernel_size=(2, 2), strides=(2, 2))(
            conv_up2
        )
...
def test_simple_unet():
    ...
    dummy_input = jnp.ones(
        (1, 128, 128, 3)
    )  # Batch size = 1, Height = 128, Width = 128, Channels = 3
