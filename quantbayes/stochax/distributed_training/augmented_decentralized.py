...
if __name__ == "__main__":
    ...
    num_samples, feature_dim = 1000, 6
    ...
    plt.title("Augmented Lagrangian decentralized — loss curve")
    plt.tight_layout()
    plt.show()

    # Ensemble vs per-node accuracies
    key_pred = jr.PRNGKey(42)
    y1 = jnp.squeeze(y_test, axis=-1).astype(jnp.int32)
    accs = []
    for nid in range(3):
        preds = trainer.predict(X_test, node=nid, key=key_pred)
        accs.append(float((preds == y1).mean()))
    preds_ens = trainer.predict(X_test, node=None, key=key_pred)
    acc_ens = float((preds_ens == y1).mean())
    for i, a in enumerate(accs):
        print(f"node {i} acc: {a*100:.2f}%")
    print(f"ensemble acc: {acc_ens*100:.2f}%")
