    ...
    * Optionally allow learning-rate schedule \ghamma_t (callable), else constant γ.
    ...
    def __init__(
        ...
        *,
        ...
        # W (fixed topology) + sanity checks
        ...
        # models/states populated in fit(...)
    ...
    def fit(
        self,
        parts: List[Tuple[Array, Array]],  # per-node (X_i, y_i)
        X_full: Array,
        y_full: Array,
        *,
        ...
        for t in range(self.T):
            ...
            # 2) Local SGD step with per-node mini-batch
            new_params_list = []
            for i in range(self.n_nodes):
                ...
                if B >= n_i:
                    ...
                    idx = jr.choice(sub, n_i, shape=(B,), replace=False)
                Xb, yb = Xi[idx], yi[idx]

                # compute stochastic gradient at $\theta_{t+1/2}^i$
                ...
                updated = jax.tree_util.tree_map(
                    lambda p, gg: p - step * gg, params_half[i], g
                )
            ...
            # 3) Logging (global training loss of nodes 1 and 4; consensus)
...
class DGDTrainerSwitchingEqx:
    ...
    def __init__(
        ...
        *,
        ...
        for W in (self.W_odd, self.W_even):
            rs = jnp.sum(W, axis=1)
            assert jnp.allclose(W, W.T, atol=1e-6), "W must be symmetric"
            assert jnp.allclose(
                rs, jnp.ones_like(rs), atol=1e-6
    ...
    def fit(
        ...
        *,
        ...
        for t in range(self.T):
            # choose topology for this iteration
