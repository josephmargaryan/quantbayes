...
class DecentralizedTrainer:
    ...
    def local_vanilla_train(
        ...
        def vanilla_loss(model, state, xb, yb, k):
            base, new_state = binary_loss(model, state, xb, yb, k)
...
if __name__ == "__main__":
    ...
    def node_accuracy(model, state, X, y, key):
        logits = predict(model, state, X, key)
        preds = (sigmoid(logits) > 0.5).astype(jnp.float32)
        return float((preds == y).mean())

    key_pred = jr.PRNGKey(42)
    for i, wi in enumerate(w_list):
        acc_i = node_accuracy(wi, None, X_test, y_test, key_pred)
        print(f"Node {i} test accuracy: {acc_i*100:.2f}%")
