...
class DecentralizedTrainer:
    ...
    def predict_proba(
        ...
        *,
        ...
        """
        if key is None:
            # split one key per node
            key = jax.random.PRNGKey(0)
        # split into enough subkeys
        keys = jax.random.split(key, len(self.w_list))

        # collect per-node probabilities
        probs = []
        for i, (w_i, s_i, k_i) in enumerate(zip(self.w_list, self.state_list, keys)):
            logits = self._predict_node(w_i, s_i, X, k_i)
            # binary vs multiclass
            if logits.shape[-1] == 1:
                p = jax.nn.sigmoid(logits).squeeze(-1)  # shape (batch,)
            else:
                p = jax.nn.softmax(logits, axis=-1)  # shape (batch, n_classes)
            probs.append(p)

        probs = jnp.stack(probs, axis=0)  # (n_nodes, batch, ...)

        if node is None:
            # ensemble: average over nodes
...
if __name__ == "__main__":
    # Simple test with random data & a 3-node line graph
    import numpy as onp
