...
def _kl_bin(p: float, q: float, eps=_EPS) -> float:
...
def _kl_inv_plus(p_hat: float, c: float, tol=1e-10) -> float:
    ...
    for _ in range(60):
        mid = (lo + hi) * 0.5
        (_kl_bin(p_hat, mid) <= c and (lo := mid)) or (hi := mid)
        if hi - lo < tol:
            break
    return lo
...
class PlainKLCriterion:
    ...
    def compute(self, losses, rho, kl, n, delta, *_):
        ...
        b = _kl_inv_plus(p_hat, (kl + math.log(2 * math.sqrt(n) / delta)) / n)
        return p_hat, b


class _SplitBase:
    """Common init for split criteria."""
...
class SplitKLCriterion(_SplitBase):
    uses_lambda = False

    def compute(self, fhat, rho, kl, n, delta, *_):
        c = (kl + math.log(2 * self.k * math.sqrt(n) / delta)) / n
        bound = self.b[0]
        for j, alpha_j in enumerate(self.alpha):
            p_hat = float(rho @ fhat[j])
            bound += alpha_j * _kl_inv_plus(p_hat, c)
        return None, bound


class SplitBernsteinCriterion(_SplitBase):
    uses_lambda = False

    def compute(self, fhat, rho, kl, n, delta, *_):
        log_term = kl + math.log(2 * self.k * math.sqrt(n) / delta)
        bound = self.b[0]
        for j, alpha_j in enumerate(self.alpha):
            p_hat = float(rho @ fhat[j])
            term = (
                p_hat
                + math.sqrt(2 * p_hat * (1 - p_hat) * log_term / n)
                + 2 * log_term / (3 * n)
            )
            bound += alpha_j * term
        return None, bound
...
class RecursivePACBayesEnsemble:
    ...
    def fit(self, X, y):
        ...
        _, B_prev = crit1.compute(L.mean(1), w_prev, kl1, len(y), δ_step)
