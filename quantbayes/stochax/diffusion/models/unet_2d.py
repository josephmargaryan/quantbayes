    ...
    def __call__(self, x, t, *, key):
        ...
        for layer in self.block2_layers:
            ...
            if isinstance(layer, eqx.nn.Dropout):
                # Disable dropout if no key was passed (e.g., sampling). This makes
                # inference deterministic even if someone forgets tree_inference.
                h = layer(h, key=key, inference=(key is None))
            else:
                h = layer(h)
...
class UNet(eqx.Module):
    ...
    def _forward(self, t, y, *, key):
        # t: scalar time (or 0D array)
        # y: single image tensor with shape (C, H, W)
