...
from quantbayes.stochax.diffusion.samplers.edm_heun import (
    sample_edm_heun,
    make_edm_heun_sampler,  # NEW: batched + JIT
)
...
def generate_with_sampler(
    ...
    *,
    ...
    Other samplers fall back to per-sample vmapped calls.
    """
    ...
    if s in ("edmheun", "heun"):
        batched = make_edm_heun_sampler(
            denoise_fn,
            sample_shape=sample_shape,
            **sampler_kwargs,
        )
        samples = batched(key, num_samples)

    elif s in ("dpmpp3m", "dpmsolverpp3m"):
        batched = make_dpmpp_3m_sampler(
            denoise_fn,
            sample_shape=sample_shape,
            **sampler_kwargs,
        )
        ...
        from quantbayes.stochax.diffusion.samplers.ipndm import make_ipndm_sampler

        batched = make_ipndm_sampler(
            denoise_fn,
            sample_shape=sample_shape,
            **sampler_kwargs,
        )
        ...
        from quantbayes.stochax.diffusion.samplers.ipndm import make_ipndm4_sampler

        batched = make_ipndm4_sampler(
            denoise_fn,
            sample_shape=sample_shape,
            **sampler_kwargs,
        )
    ...
    # ----------------------------
    # Per-sample vmapped fallbacks
    # ----------------------------
    else:
        if s in ("dpmpp2m", "dpmsolverpp2m"):

            def fn(k):
                return sample_dpmpp_2m(
                    denoise_fn, sample_shape, key=k, **sampler_kwargs
                )
