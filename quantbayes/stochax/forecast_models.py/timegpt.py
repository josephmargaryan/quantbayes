...
class TimeGPT(eqx.Module):
    ...
    def __init__(self, num_layers: int, embed_dim: int, num_heads: int,
        ...
        self.blocks = [TimeGPTBlock(embed_dim, num_heads, mlp_ratio, dropout_p, key=k)
                       for k in keys[1:-1]]
...
class TimeGPTForecast(eqx.Module):
    ...
    def __init__(self, num_layers: int, embed_dim: int, num_heads: int,
                 mlp_ratio: float, dropout_p: float, max_len: int, *, key):
        self.model = TimeGPT(num_layers, embed_dim, num_heads, mlp_ratio, dropout_p, max_len, key=key)
...
if __name__ == '__main__':
    ...
    model = TimeGPTForecast(num_layers=4, embed_dim=D, num_heads=4,
                            mlp_ratio=4.0, dropout_p=0.1, max_len=50, key=model_key)
