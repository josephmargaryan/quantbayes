...
class MulticlassClassificationModel:
    ...
    def _compute_loss(self, model, state, X, y, key, training=True):
        ...
        all_logits = jax.vmap(forward_one, in_axes=(0, 0))(X, keys)  # shape [batch, num_classes]
    ...
    def visualize(self, model, state, X_test, y_test, feature_indices=(0, 1), title="Decision Boundary"):
        """
...
if __name__ == "__main__":
    ...
    trainer.visualize(model, state, jnp.array(X_test), np.array(y_test), feature_indices=(0,1), title="Multiclass Classification Demo")
