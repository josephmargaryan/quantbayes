...
# C+R UB: add padding arg and use eigvalsh for speed (keep your chunking if you like)
def _sigma_conv_circ_plus_lr_upper_2d(
    ...
    *,
    ...
    eW = (kW - 1) * dW + 1
    Hc, Wc = H + eH - 1, W + eW - 1
    Kd = jnp.zeros((Co, Ci, eH, eW), dtype=K.dtype).at[..., ::dH, ::dW].set(K)
    Kf = jnp.fft.rfft2(Kd, s=(Hc, Wc), axes=(-2, -1), norm="ortho")
    mats = jnp.transpose(Kf, (2, 3, 0, 1)).reshape(-1, Co, Ci)

    def _svmax(M):
        G = M @ jnp.conjugate(M.T)
        lam = jnp.linalg.eigvalsh(G)[-1]
        return jnp.sqrt(jnp.real(lam))

    circ = jnp.sqrt(Hc * Wc) * jnp.max(jax.vmap(_svmax)(mats))
    B = _border_B(H, W, kH, kW, sH, sW, dH, dW, padding=padding)
    return circ + jnp.sqrt(jnp.asarray(B, dtype=K.real.dtype)) * jnp.linalg.norm(K)
