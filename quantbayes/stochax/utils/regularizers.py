...
def _join_module_path(parent: str, child: str) -> str:
    if not parent:
        return child
    if child.startswith("["):
...
def collect_operator_norms(
    model: Any,
    *,
    conv_mode: Literal["tn", "circular_fft", "circular_gram"] = "tn",
    conv_tn_iters: int = 8,
    conv_gram_iters: int = 5,
    conv_fft_shape: Optional[Tuple[int, int]] = None,
    allow_exact_hints_for: Tuple[str, ...] = (
    ...
    """Collect per-layer operator-norm diagnostics.

    The intent is *model-only* diagnostics/logging for already-trained DP checkpoints.
    For convolutional layers the default `conv_mode='tn'` uses the certified tensor-norm
    upper bound, which is resolution-free and works without an input-shape trace.

    Notes
    -----
    - This logger intentionally focuses on affine / convolutional layers and layers that
      expose `__operator_norm_hint__()`.
    - For general branched architectures, use `conv_mode='tn'` unless you can certify a
      per-layer input resolution externally and log with a custom helper.
    """
    rows: list[dict[str, Any]] = []
    nn = getattr(eqx, "nn", None)
    param_svd_conv_names = {"SpectralConv2d", "AdaptiveSpectralConv2d"}

    def _record(path: str, layer_type: str, sigma: float, source: str) -> None:
        rows.append(
            {
                "path": path,
    ...
    def _conv_sigma(
        weight: jnp.ndarray, strides: tuple[int, int], dilations: tuple[int, int]
    ):
        if conv_mode == "circular_fft":
            if strides == (1, 1) and conv_fft_shape is not None:
                return _sigma_conv_circular_fft_exact_2d(
    ...
    def visit(x: Any, path: str) -> None:
        ...
        if (name in allow_exact_hints_for) and hasattr(x, "__operator_norm_hint__"):
            try:
                sigma = x.__operator_norm_hint__()
                if sigma is not None:
                    _record(path, name, float(jnp.asarray(sigma)), "operator_norm_hint")
                    return
            except Exception:
                pass

        if nn is not None and isinstance(x, getattr(nn, "Linear", ())):
            W = jnp.asarray(x.weight).reshape(x.weight.shape[0], -1)
            sigma = jnp.linalg.svd(W, compute_uv=False)[0]
            _record(path, name, float(jnp.asarray(sigma)), "exact_svd")
            return
        ...
        if nn is not None and isinstance(
            ...
            K = jnp.asarray(x.weight)
            num_dims = int(getattr(x, "num_spatial_dims", K.ndim - 2))
            if num_dims == 2:
                s = getattr(x, "stride", getattr(x, "strides", (1, 1)))
                s = (s, s) if isinstance(s, int) else s
                d = (
                ...
                d = (d, d) if isinstance(d, int) else d
                if "ConvTranspose" in name and K.shape[0] < K.shape[1]:
                ...
                _record(path, name, float(jnp.asarray(sigma)), f"conv_{conv_mode}")
                return

        if name in param_svd_conv_names:
            W_mat = x.U @ (x.s[:, None] * x.V.T)
            K = jnp.reshape(W_mat, (x.C_out, x.C_in, x.H_k, x.W_k))
            s = getattr(x, "strides", (1, 1))
            s = (s, s) if isinstance(s, int) else s
            sigma = _conv_sigma(K, (int(s[0]), int(s[1])), (1, 1))
            _record(path, name, float(jnp.asarray(sigma)), f"svd_conv_{conv_mode}")
            return

        if isinstance(x, eqx.Module):
            for child_name, child_value in vars(x).items():
                visit(child_value, _join_module_path(path, str(child_name)))
            return
        if isinstance(x, (list, tuple)):
            for i, child_value in enumerate(x):
                visit(child_value, _join_module_path(path, f"[{i}]"))
            return
        if isinstance(x, dict):
            for child_key, child_value in x.items():
                visit(child_value, _join_module_path(path, f"[{child_key!r}]"))
            return
...
def summarize_operator_norms(rows: list[dict[str, Any]]) -> dict[str, Any]:
    sigmas = [float(r["sigma"]) for r in rows if r.get("sigma") is not None]
    out = {
        "num_layers": int(len(rows)),
        "num_valid_sigma_layers": int(len(sigmas)),
        "max_sigma": None,
        "mean_sigma": None,
    }
    if sigmas:
        arr = np.asarray(sigmas, dtype=np.float64)
        out["max_sigma"] = float(np.max(arr))
        out["mean_sigma"] = float(np.mean(arr))
    return out
