...
def cast_tree(tree: Any, dtype=jnp.bfloat16, keep_fp32_predicate=None) -> Any:
    ...
    def _cast(path, x):
        if not eqx.is_inexact_array(x):
            return x
        if keep_fp32_predicate is not None and keep_fp32_predicate(path, x):
            return x
    ...
    return eqx.tree_map_with_path(mapper, tree)
...
def make_optimizer_with_clipping(
    ...
    """
    Wrap an existing optax optimizer with global-norm gradient clipping.
    Example:
        optimizer = make_optimizer_with_clipping(
            optax.adamw(1e-3, weight_decay=1e-4), clip_global_norm=1.0
        )
    """
    if clip_global_norm is None or clip_global_norm <= 0:
        return base
    return optax.chain(optax.clip_by_global_norm(clip_global_norm), base)
