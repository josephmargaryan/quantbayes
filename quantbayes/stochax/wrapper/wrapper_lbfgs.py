# quantbayes/stochax/wrapper/wrapper_lbfgs.py
...
from quantbayes.stochax.trainer.quasi_newton import (
    train_lbfgs as _train_lbfgs,
    train_lbfgs_full_data as _train_lbfgs_full,
)
...
class _BaseLBFGS(BaseEstimator):
    """
    Sklearn-compatible LBFGS wrapper base for Equinox models.
    We keep the interface parallel to the first-order wrappers,
    sans EMA/SAM, and expose the L-BFGS knobs.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # checkpoints
        ckpt_path: Optional[str] = None,  # "ckpts/run-epoch-{epoch:03d}.ckpt"
    ...
    def _more_tags(self):
        # We expect 2D X in these LBFGS wrappers.
    ...
    def fit(self, X, y):
        ...
        # 5) optional refit on full data for best_epoch_ epochs
        if self.refit_on_full_data:
            master, rf_key = jr.split(master)
            out_full = _train_lbfgs_full(
                model=best_model,
                state=best_state,
                X_train=X_tr,
                y_train=y_tr,
...
class EQXRegressorLBFGS(_BaseLBFGS, RegressorMixin):
...
class EQXBinaryClassifierLBFGS(_BaseLBFGS, ClassifierMixin):
    def __init__(self, **kwargs):
        if "loss_fn" not in kwargs or kwargs["loss_fn"] is None:
...
class EQXMulticlassClassifierLBFGS(_BaseLBFGS, ClassifierMixin):
    def __init__(self, **kwargs):
        if "loss_fn" not in kwargs or kwargs["loss_fn"] is None:
