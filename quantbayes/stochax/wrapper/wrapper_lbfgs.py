...
class EQXBaseLBFGS(BaseEstimator):
    def __init__(
        self,
    ...
    def get_params(self, deep=True):
        # return exactly the __init__ signature
        return {
    ...
    def _fit(
        ...
        # record for scikit-learn compatibility
        # (so Pipeline, check_array, etc., all know feature count)
        ...
        # 4) init model + state
        #   use empty dict if user passed None
...
class EQXRegressorLBFGS(EQXBaseLBFGS, RegressorMixin):
    def fit(self, X, y):
        # regression: float32, shape (n_samples, 1)
        y_arr = np.asarray(y, dtype=np.float32).reshape(-1)
        return self._fit(X, y_arr, train_fn=train_lbfgs, loss_fn=regression_loss)
...
class EQXMulticlassClassifierLBFGS(EQXBaseLBFGS, ClassifierMixin):
    def fit(self, X, y):
        # multiclass: int32 labels shape (n_samples,)
        y_arr = np.asarray(y, dtype=np.int32).reshape(-1)
        self.classes_ = np.unique(y_arr)
        return self._fit(X, y_arr, train_fn=train_lbfgs, loss_fn=multiclass_loss)
