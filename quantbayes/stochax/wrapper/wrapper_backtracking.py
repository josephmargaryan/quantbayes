...
from quantbayes.stochax.trainer.backtracking import (
    train_backtrack as _train_backtrack,
    train_backtrack_full_data as _train_backtrack_full,
)
...
class _BaseBacktrack(BaseEstimator):
    """
    Sklearn-compatible wrapper for backtracking line-search (SGD direction).
    Mirror the LBFGS wrappers: deterministic objective, penalties, checkpoints,
    optional refit on full data, and stable proba utilities.
    """
    ...
    def fit(self, X, y):
        ...
        if self.refit_on_full_data:
            ...
            out_full = _train_backtrack_full(
                model=best_model,
                state=best_state,
                X_train=X_tr,
                y_train=y_tr,
...
# —— Regressor ——
class EQXRegressorBacktrack(_BaseBacktrack, RegressorMixin):
...
# —— Binary classifier ——
class EQXBinaryClassifierBacktrack(_BaseBacktrack, ClassifierMixin):
    def __init__(self, **kwargs):
        if "loss_fn" not in kwargs or kwargs["loss_fn"] is None:
...
class EQXMulticlassClassifierBacktrack(_BaseBacktrack, ClassifierMixin):
    def __init__(self, **kwargs):
        if "loss_fn" not in kwargs or kwargs["loss_fn"] is None:
