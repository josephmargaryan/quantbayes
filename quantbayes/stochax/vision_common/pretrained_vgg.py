...
def _warmstart_svddense(mod: Any, W: jnp.ndarray, b: Optional[jnp.ndarray]):
    U, s, V = _svd_linear(W)
    r = min(len(s), getattr(mod, "s").shape[0])
    U = U[:, :r].astype(mod.U.dtype)
    V = V[:, :r].astype(mod.V.dtype)
...
def _warmstart_svdconv(mod: Any, W: jnp.ndarray, b: Optional[jnp.ndarray], report):
    ...
    report["n_loaded"] += int(W.size) + (int(b.size) if (b is not None) else 0)
    return new
