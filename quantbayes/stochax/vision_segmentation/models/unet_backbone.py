...
class BasicBlock(eqx.Module):
    ...
    def __init__(self, cin: int, cout: int, stride: int, *, key):
...
class ResNetEncoder(eqx.Module):
    ...
    def __init__(self, backbone: str, *, key):
        ...
        def _make_layer(cin, cout, blocks, stride, key_iter):
            mods = []
            c_in = cin
            for i in range(blocks):
                s = stride if i == 0 else 1
                mods.append(Block(c_in, cout, s, key=next(key_iter)))
                c_in = cout * (4 if Block is Bottleneck else 1)
            return tuple(mods), c_in

        k_iter = iter(ks[1:])

        self.layers1, ch1 = _make_layer(64, 64, layer_sizes[0], 1, k_iter)
        self.layers2, ch2 = _make_layer(ch1, 128, layer_sizes[1], 2, k_iter)
        self.layers3, ch3 = _make_layer(ch2, 256, layer_sizes[2], 2, k_iter)
        self.layers4, _ = _make_layer(ch3, 512, layer_sizes[3], 2, k_iter)
