...
class BasicBlock(eqx.Module):
    ...
    def __init__(self, cin: int, cout: int, stride: int = 1, *, key):
        ...
        self.conv1 = eqx.nn.Conv2d(cin, cout, 3, stride=stride, padding=1, use_bias=False, key=k1)
        ...
        if stride != 1 or cin != cout:
            self.down = eqx.nn.Conv2d(cin, cout, 1, stride=stride, use_bias=False, key=k3)
...
class Bottleneck(eqx.Module):
    ...
    def __init__(self, cin: int, planes: int, stride: int = 1, *, key):
        ...
        self.bn1 = eqx.nn.BatchNorm(cout, axis_name="batch", mode="batch")
        self.conv2 = eqx.nn.Conv2d(cout, cout, 3, stride=stride, padding=1, use_bias=False, key=k2)
        self.bn2 = eqx.nn.BatchNorm(cout, axis_name="batch", mode="batch")
        self.conv3 = eqx.nn.Conv2d(cout, cout * self.expansion, 1, use_bias=False, key=k3)
        self.bn3 = eqx.nn.BatchNorm(cout * self.expansion, axis_name="batch", mode="batch")
        if stride != 1 or cin != cout * self.expansion:
            self.down = eqx.nn.Conv2d(cin, cout * self.expansion, 1, stride=stride, use_bias=False, key=k4)
            self.down_bn = eqx.nn.BatchNorm(cout * self.expansion, axis_name="batch", mode="batch")
        else:
            self.down, self.down_bn = None, None

    def __call__(self, x, key, state):
        y = self.conv1(x, key=jr.fold_in(key, 1)); y, state = self.bn1(y, state); y = jax.nn.relu(y)
        y = self.conv2(y, key=jr.fold_in(key, 2)); y, state = self.bn2(y, state); y = jax.nn.relu(y)
        y = self.conv3(y, key=jr.fold_in(key, 3)); y, state = self.bn3(y, state)
        if self.down is not None:
            skip = self.down(x, key=jr.fold_in(key, 4)); skip, state = self.down_bn(skip, state)
        else:
            skip = x
        out = jax.nn.relu(y + skip)
        return out, state
...
class HighResolutionModule(eqx.Module):
    ...
    def __init__(self, num_branches: int, blocks: int, channels: Tuple[int, ...], block_type, *, key):
        """
        ...
        for i in range(num_branches):
            ...
            for j in range(num_branches):
                if i == j:
                    ...
                    row.append(eqx.nn.Conv2d(channels[j], channels[i], 1, use_bias=False, key=next(branch_keys)))
                else:
                    ...
                    for k in range(i - j):
                        ...
                        ops.append(eqx.nn.Conv2d(in_c, out_c, 3, stride=2, padding=1, use_bias=False, key=next(branch_keys)))
                        in_c = out_c
    ...
    def __call__(self, xs: Tuple[jnp.ndarray, ...], key, state) -> Tuple[Tuple[jnp.ndarray, ...], Any]:
...
class HRNetBackbone(eqx.Module):
    ...
    def __init__(self, *, arch: str = "hrnet_w18_small_v2", key):
        ...
        self.stem_conv1 = eqx.nn.Conv2d(3, 64, 3, stride=2, padding=1, use_bias=False, key=next(k_iter))
        self.stem_bn1 = eqx.nn.BatchNorm(64, axis_name="batch", mode="batch")
        self.stem_conv2 = eqx.nn.Conv2d(64, 64, 3, stride=2, padding=1, use_bias=False, key=next(k_iter))
        self.stem_bn2 = eqx.nn.BatchNorm(64, axis_name="batch", mode="batch")

        # layer1 (Bottleneck x 4; output 256 channels)
        self.layer1, c_after_l1 = _make_layer(Bottleneck, 64, 64, blocks=4, key=next(k_iter))

        # transition to stage2 (2 branches)
        self.trans2 = (
            eqx.nn.Conv2d(c_after_l1, widths[0], 3, padding=1, use_bias=False, key=next(k_iter)),
            eqx.nn.Conv2d(c_after_l1, widths[1], 3, stride=2, padding=1, use_bias=False, key=next(k_iter)),
        )
        self.stage2 = HighResolutionModule(num_branches=2, blocks=blocks[0], channels=(widths[0], widths[1]), block_type="basic", key=next(k_iter))

        # transition to stage3 (3 branches)
        self.trans3 = (
            eqx.nn.Conv2d(widths[0], widths[0], 3, padding=1, use_bias=False, key=next(k_iter)),  # keep
            eqx.nn.Conv2d(widths[1], widths[1], 3, padding=1, use_bias=False, key=next(k_iter)),  # keep
            eqx.nn.Conv2d(widths[1], widths[2], 3, stride=2, padding=1, use_bias=False, key=next(k_iter)),  # new down
        )
        self.stage3 = HighResolutionModule(num_branches=3, blocks=blocks[1], channels=(widths[0], widths[1], widths[2]), block_type="basic", key=next(k_iter))

        # transition to stage4 (4 branches)
        self.trans4 = (
            eqx.nn.Conv2d(widths[0], widths[0], 3, padding=1, use_bias=False, key=next(k_iter)),  # keep
            eqx.nn.Conv2d(widths[1], widths[1], 3, padding=1, use_bias=False, key=next(k_iter)),  # keep
            eqx.nn.Conv2d(widths[2], widths[2], 3, padding=1, use_bias=False, key=next(k_iter)),  # keep
            eqx.nn.Conv2d(widths[2], widths[3], 3, stride=2, padding=1, use_bias=False, key=next(k_iter)),  # new down
        )
        self.stage4 = HighResolutionModule(num_branches=4, blocks=blocks[2], channels=(widths[0], widths[1], widths[2], widths[3]), block_type="basic", key=next(k_iter))
    ...
    def __call__(self, x, key, state):
        # stem
        x = self.stem_conv1(x, key=jr.fold_in(key, 1)); x, state = self.stem_bn1(x, state); x = jax.nn.relu(x)
        x = self.stem_conv2(x, key=jr.fold_in(key, 2)); x, state = self.stem_bn2(x, state); x = jax.nn.relu(x)
        # layer1
        ...
        (x0, x1, x2), state = self.stage3((x0, x1, x2), key=jr.fold_in(key, 10), state=state)
        ...
        feats, state = self.stage4((x0, x1, x2, x3), key=jr.fold_in(key, 15), state=state)
...
class SpatialGather(eqx.Module):
    ...
    def __call__(self, feats_chw: jnp.ndarray, class_logits_chw: jnp.ndarray):
        ...
        probs = jax.nn.softmax(einops.rearrange(class_logits_chw, 'k h w -> (h w) k'), axis=-1)  # [N, K]
        weights = probs / jnp.clip(jnp.sum(probs, axis=0, keepdims=True), 1e-6)  # [N,K] normalized per class
...
class ObjectAttention(eqx.Module):
    ...
    def __init__(self, in_channels: int, key_channels: int, value_channels: int, out_channels: int, *, key):
        k1, k2, k3, k4 = jr.split(key, 4)
        ...
        self.out = eqx.nn.Conv2d(value_channels, out_channels, 1, use_bias=False, key=k4)
...
class OCRHead(eqx.Module):
    ...
    def __init__(self, in_channels: int, key_channels: int, value_channels: int, num_classes: int, *, key):
        k1, k2, k3 = jr.split(key, 3)
        self.conv3x3 = eqx.nn.Conv2d(in_channels, 512, 3, padding=1, use_bias=False, key=k1)
        ...
        self.obj_attn = ObjectAttention(512, key_channels, value_channels, out_channels=512, key=k2)
...
class HRNetOCR(eqx.Module):
    ...
    def __init__(self, *, arch: str = "hrnet_w18_small_v2", num_classes: int = 19, key):
        ...
        # OCR head
        self.ocr = OCRHead(in_channels=512, key_channels=256, value_channels=256, num_classes=num_classes, key=k3)
...
if __name__ == "__main__":
    ...
    plt.xlabel("epoch"); plt.ylabel("loss"); plt.legend(); plt.title("HRNet-OCR smoke test")
    plt.show()
