...
class ConvBlock(eqx.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, *, key: jr.PRNGKey):
    ...
    def __call__(
        self, x: jnp.ndarray, state: eqx.nn.State, *, key: jr.PRNGKey
...
class UpConvBlock(eqx.Module):
    ...
    def __call__(
        self, x: jnp.ndarray, skip: jnp.ndarray, state: eqx.nn.State, *, key: jr.PRNGKey
    ) -> Tuple[jnp.ndarray, eqx.nn.State]:
        k1, k2 = jr.split(key, 2)
        x = self.upconv(x, key=k1)
        x = jnp.concatenate([skip, x], axis=0)  # channel‐first
...
class UNet(eqx.Module):
    ...
    def __call__(
        self, x: jnp.ndarray, key: jr.PRNGKey, state: eqx.nn.State
    ) -> Tuple[jnp.ndarray, eqx.nn.State]:
        # split into 9 subkeys: 4×enc, 1×bottleneck, 4×dec
