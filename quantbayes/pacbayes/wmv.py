...
class SplitBernsteinCriterion(BoundCriterion):
    """Split-Empirical-Bernstein for finite-valued losses."""
    ...
    def __init__(self, gamma: float):
        ...
        self.b = b
        self.alpha = np.diff(b)
        self.k_slices = len(self.alpha)

    def compute(self, fhat_slices, rho, kl, n, delta, *_):
        """Return (None, bound) just like SplitKL."""
        logc = (kl + math.log(self.k_slices * math.sqrt(n) / delta)) / n
        bound = self.b[0]
        for j, alpha_j in enumerate(self.alpha):
            p_hat = float(rho @ fhat_slices[j])
            # empirical-Bernstein PAC-Bayes term
            term = (
                p_hat + math.sqrt(2.0 * p_hat * (1.0 - p_hat) * logc) + 2.0 * logc / 3.0
            )
            bound += alpha_j * term
        return None, bound
