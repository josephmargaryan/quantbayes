...
# ----------------------------------------------------------------------------
# 3. Evaluate (Compute IoU + Optional MC)
# ----------------------------------------------------------------------------
def compute_iou(
    ...
    We threshold pred_mask -> compute intersection & union.
    """
...
def evaluate_model(
    ...
    """
    Compute average IoU across the entire dataset. For MC dropout, set model.train() repeatedly.
    """
    ...
    with torch.no_grad():
        for batch_x, batch_y in mini_batches(X_val_t, Y_val_t, batch_size):
            ...
            for _ in range(num_samples):
                model.eval()  # or train() for MC dropout
                logits = model(batch_x)  # (b,1,H,W)
                prob = torch.sigmoid(logits)  # (b,1,H,W)
                all_probs.append(prob)
            # average
            mean_prob = torch.mean(torch.stack(all_probs, dim=0), dim=0)  # (b,1,H,W)
            # compute IoU
            b, _, H, W = mean_prob.shape
            mean_prob_2d = mean_prob.view(b, H, W)
            batch_y_2d = batch_y.view(b, H, W)
            batch_iou = compute_iou(mean_prob_2d, batch_y_2d)
...
def visualize_segmentation(
    ...
    """
    Plot side by side: image, predicted mask, ground truth.
    ...
    for i, idx in enumerate(idxs):
        ...
        # convert img to (H,W,3) for plotting
...
if __name__ == "__main__":
    ...
    # evaluate
    iou_val = evaluate_model(model, X_val, Y_val, batch_size=8, num_samples=1)
