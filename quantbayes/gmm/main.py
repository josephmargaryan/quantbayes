...
def generate_synthetic_data(
    num_samples=500, locs=[2.0, 8.0], scales=[1.0, 2.0], weights=[0.4, 0.6]
):
    key = random.PRNGKey(42)
    component_ids = random.categorical(
        key, jnp.log(jnp.array(weights)), shape=(num_samples,)
    )
    data = jnp.array(
        [
...
data = generate_synthetic_data(
    num_samples=500, locs=locs, scales=scales, weights=weights
)
