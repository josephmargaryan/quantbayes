...
if use_custom_guides:
    ...
    K_value = 9
    spectral_real_guide = LowRankSpectralGuide(
        model, K=K_value, site_name="spectral_circ_jvp_real", rank=5
    )
    spectral_imag_guide = LowRankSpectralGuide(
        model, K=K_value, site_name="spectral_circ_jvp_imag", rank=5
    )
    other_guide = AutoNormal(
        numpyro.handlers.block(
            model, hide=["spectral_circ_jvp_real", "spectral_circ_jvp_imag"]
        )
    )
...
loss_grid, scales = visualize_loss_landscape(
    compute_loss,
    params,
    model,
    guide,
...
def visualize_loss_landscape_3d(loss_grid, scales, title="3D Loss Landscape"):
    ...
    surf = ax.plot_surface(
        X_grid, Y_grid, loss_grid, cmap=cm.viridis, edgecolor="none", alpha=0.8
    )
...
def interpolate_loss(
    loss_fn, init_params, final_params, model, guide, X, y, num_points=21, rng_key=None
):
    """
...
alphas, interp_losses = interpolate_loss(
    compute_loss,
    init_params,
    params,
    model,
...
plt.plot(
    smooth_curve(losses, window=50), label="Smoothed Loss (window=50)", linewidth=2
)
...
H_proj = compute_projected_hessian(
    compute_loss, params, model, guide, X_train, y_train, viz_key, directions
)
...
######################################
def multiple_projections_diagnostics(
    loss_fn,
    params,
    model,
    guide,
    ...
    for seed in range(num_projections):
        ...
        for i, a in enumerate(scales):
            for j, b in enumerate(scales):
                ...
                loss_grid[i, j] = np.array(
                    loss_fn(params_perturbed, model, guide, X, y, viz_key)
                )
...
multiple_projections_diagnostics(
    compute_loss,
    params,
    model,
    guide,
...
######################################
def interpolation_with_curvature(
    loss_fn, init_params, final_params, model, guide, X, y, num_points=21, rng_key=None
):
    flat_init, unravel_fn = ravel_pytree(init_params)
    flat_final, _ = ravel_pytree(final_params)
    ...
    plt.subplot(2, 1, 2)
    plt.plot(
        alphas, second_deriv, marker="o", label="Estimated Curvature (2nd Derivative)"
    )
...
alphas_interp, losses_interp, curvature_interp = interpolation_with_curvature(
    compute_loss,
    init_params,
    params,
    model,
...
predictive_mean, predictive_std = uncertainty_quantification(
    predictive, X_test, y_test, num_samples=100
)
