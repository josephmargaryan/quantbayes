...
from .attacks.trace_optimization import (
...
def fit_convex(
    ...
    *,
    ...
    """Fit a convex release with the stable public API.
    ...
    X_train, y_train:
        Training data.
    model_family:
        One of ``'ridge_prototype'``, ``'softmax_logreg'``, ``'binary_logreg'``,
        or ``'svm'`` (plus the documented aliases).
    radius:
        Ball-adjacency radius ``r``. If you want the Ball view to coincide with the
        usual replacement-adjacent comparator for the theorem-backed convex bounds,
        pass ``radius = 2 * embedding_bound``.
    lam:
        Regularization strength in the convex ERM objective.
    X_eval, y_eval:
        Optional public evaluation set used only for utility metrics.
    privacy:
        One of ``'ball_dp'``, ``'ball_rdp'``, or ``'noiseless'``. Aliases
        ``'dp'`` and ``'rdp'`` map to the Ball variants.
    epsilon, delta:
        Required for ``privacy='ball_dp'``.
    sigma:
        Required for ``privacy='ball_rdp'``.
    embedding_bound:
        User-supplied bound ``||x_i||_2 <= B``. The name is historical; in image
        or tabular settings this is simply an input-feature norm bound.
    num_classes:
        Optional class count for multiclass families.
    lz:
        Optional explicit theorem-backed Lipschitz constant. If omitted, the
        low-level convex code uses its built-in family-specific bound.
    standard_radius:
        Optional comparator radius to record on the standard ledger.
    gaussian_method, gaussian_tol, orders, dp_deltas_for_rdp:
        Privacy-accounting controls.
    solver:
        Public convex API intentionally exposes only deterministic full-batch
        solvers: ``'lbfgs_fullbatch'`` and ``'gd_fullbatch'``.
    max_iter, learning_rate, grad_tol, param_tol, objective_tol, line_search_steps:
        Numerical solver controls.
    seed:
        Random seed for model initialization and Gaussian noise.

    Returns
    -------
    ReleaseArtifact
        The trained release plus privacy/accounting/utility metadata.

    Notes
    -----
    The public API intentionally hides low-level configuration flags such as
    ``theorem_backed_only``, ``certify_approximation``, and
    ``store_nonprivate_reference``. Internally we keep the deterministic,
    theorem-oriented path and record the usual residual-based approximation
    heuristic for iterative solvers.
    """
...
def attack_convex(
    ...
    *,
    ...
    """Run the direct convex reconstruction attack for one target point.

    Parameters
    ----------
    release:
        Output of :func:`fit_convex` or a compatible low-level convex release.
    X_train, y_train:
        The original training data used to form ``D^-`` and the target point.
    target_index:
        Index of the target record to reconstruct.
    known_label:
        Optional side information for attacks that can exploit the target label.
    eta_grid:
        Thresholds used to populate ``success@eta`` metrics.

    Returns
    -------
    attack_result, d_minus, true_record

    Notes
    -----
    The most important fields on the returned :class:`AttackResult` are:

    - ``status``: which algebraic branch / fallback the attack took. This is a
      diagnostic string, not a privacy guarantee.
    - ``metrics['feature_l2']``: plain Euclidean distance ``||z_hat - z||_2``.
    - ``metrics['distance']``: label-preserving distance. If the predicted label
      is wrong, this metric is effectively treated as failure.
    - ``metrics['label_correct']``: 0/1 indicator.
    - ``metrics[f'success@{eta}']``: 0/1 indicator that the label-preserving
      distance is at most ``eta``.
    """
...
def make_uniform_ball_prior(
    *, center: np.ndarray, radius: float, dimension: Optional[int] = None
    ...
    """Build a uniform prior over an ℓ2 ball for Ball-ReRo evaluation."""
...
def make_empirical_ball_prior(
    ...
    *,
    ...
    """Build an empirical prior from candidate points.

    If ``label`` is supplied together with ``y``, the prior is restricted to that
    class before optional truncation via ``max_samples``.
    """
...
def ball_rero(
    ...
    *,
    ...
    """Compute a Ball-ReRo report from a release artifact and a prior."""
...
def get_public_curve_history(release: ReleaseArtifact) -> list[dict[str, Any]]:
    """Return the public evaluation history stored on a release artifact.

    This is intended for custom user-side plotting. The returned rows are copies.
    """
...
def get_operator_norm_history(release: ReleaseArtifact) -> list[dict[str, Any]]:
    """Return the operator-norm history stored on a release artifact.
...
def get_release_step_table(release: ReleaseArtifact) -> list[dict[str, Any]]:
    """Return per-step training/accounting metadata as a list of rows.

    Each row contains, when available, the batch size, clip norm, noise multiplier,
    effective noise standard deviation, Ball sensitivity, standard sensitivity,
    sensitivity ratio, and the saturation diagnostic ``rho = (L_z r) / (2 C_t)``.

    The function is read-only and does not depend on any optional plotting stack.
    """
...
def fit_ball_sgd(
    ...
    *,
    ...
    """Fit a nonconvex release with Ball-SGD / standard DP-SGD.

    Parameters
    ----------
    model, state:
        User-supplied Equinox model and optional read-only state.
    optimizer:
        The underlying Optax optimizer. The library sanitizes the private gradient
        first, then passes that sanitized gradient to ``optimizer.update(...)``.
    X_train, y_train:
        Training data.
    radius:
        Ball-adjacency radius ``r``.
    lz:
        User-supplied theorem-backed constant controlling the Ball sensitivity
        ``min(L_z r, 2 C_t)``. Pass ``None`` only when you do not need Ball
        accounting, e.g. ``privacy='standard_dp'`` or ``'standard_rdp'``.
    X_eval, y_eval:
        Optional *public* evaluation set. Public checkpoint selection and public
        metric plots should use only this dataset.
    privacy:
        One of ``'ball_dp'``, ``'ball_rdp'``, ``'standard_dp'``,
        ``'standard_rdp'``, or ``'noiseless'``.
    epsilon, delta:
        Required for ``privacy in {'ball_dp', 'standard_dp'}``.
    num_steps, batch_size, clip_norm, noise_multiplier:
        Training schedule. ``noise_multiplier`` follows the standard DP-SGD
        convention: at step ``t`` the Gaussian noise added to the *summed clipped
        gradient* has standard deviation ``noise_multiplier[t] * clip_norm[t]``.
        If ``normalize_noisy_sum_by='batch_size'``, the released average-gradient
        view therefore has per-coordinate standard deviation
        ``noise_multiplier[t] * clip_norm[t] / batch_size[t]``.
    loss_name, loss_fn, predict_fn:
        Either use the built-in classification losses via ``loss_name`` or pass a
        custom per-example loss ``loss_fn(model, state, x, y, key) -> scalar``.
        ``predict_fn`` is used only for public evaluation.
    parameter_regularizer:
        Optional parameter-only regularizer callback ``R(model, state)`` added
        *after* DP sanitization. Use this for penalties that depend only on the
        current parameters. Data-dependent regularizers belong inside ``loss_fn``.
    normalize_noisy_sum_by:
        ``'batch_size'`` or ``'none'``.
    frobenius_reg_strength, spectral_reg_strength:
        Built-in parameter-only penalties added after DP sanitization.
    spectral_reg_kwargs:
        Extra keyword arguments passed to
        ``quantbayes.stochax.utils.regularizers.global_spectral_norm_penalty``.
        Example for a convnet:
        ``{'apply_to': ('conv', 'linear'), 'conv_mode': 'tn', 'conv_tn_iters': 8}``.
    record_operator_norms, operator_norms_every, operator_norm_kwargs:
        Model-only diagnostics for saved checkpoints. ``operator_norm_kwargs`` are
        passed to ``collect_operator_norms``. Example:
        ``{'conv_mode': 'tn', 'conv_tn_iters': 8}``.
    checkpoint_selection:
        One of ``'last'``, ``'best_public_eval_loss'``, or
        ``'best_public_eval_accuracy'``.
    eval_every, eval_batch_size:
        Public evaluation cadence.
    warn_if_ball_equals_standard:
        If ``True``, emit a warning when ``min(L_z r, 2 C_t) = 2 C_t`` at every step,
        meaning the Ball and standard sensitivities coincide for the entire schedule.
    seed, key:
        PRNG controls. If ``key`` is not supplied, ``seed`` is used.
    return_debug_history:
        If ``True``, return ``(release, debug_history)`` where ``debug_history``
        contains raw train-time clipping diagnostics computed from private gradients.
        Treat this as internal-only and do not publish it without separate privacy
        treatment.

    Returns
    -------
    ReleaseArtifact or (ReleaseArtifact, dict)
        The trained release, optionally together with internal debug history.
    """
    privacy_kind = _normalize_privacy_kind(privacy)
...
def build_nonconvex_shadow_corpus(
    *,
    ...
    """Build a shadow corpus for the informed-adversary final-model attack."""
...
def fit_shadow_reconstructor(
    ...
    """Train the final-model shadow reconstructor."""
...
def attack_nonconvex_model_based(
    ...
    *,
    ...
    """Run the informed-adversary final-model attack for a nonconvex release."""
...
def make_gradient_observation(
    ...
    *,
    ...
    """Wrap an observed gradient for a gradient-inversion baseline."""
...
def attack_nonconvex_gradient_baseline(
    observation: GradientObservation,
    *,
    method: str,
    **kwargs,
) -> AttackResult:
    """Dispatch to one of the single-record gradient-inversion baselines."""
    key = str(method).strip().lower()
    if key == "dlg":
        return run_dlg_attack(observation, **kwargs)
    if key == "geiping":
        return run_geiping_attack(observation, **kwargs)
    if key == "gradinversion":
        return run_gradinversion_attack(observation, **kwargs)
    raise ValueError(f"Unknown gradient attack baseline: {method!r}")
...
def attack_nonconvex_prior_aware_trace(
    ...
    **kwargs,
    ...
    """Run the prior-aware DP-SGD trace-ranking attack.

    If ``dataset`` and ``target_index`` are provided, the wrapper first subtracts all known non-target
    contributions from each privatized step before ranking the finite prior.
    """
...
def attack_nonconvex_trace_optimization(
    ...
    *,
    ...
    """Run the continuous multi-step trace-optimization attack."""
