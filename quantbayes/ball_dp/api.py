...
from .attacks.finite_prior_setup import (
    FinitePriorTrial,
...
def attack_convex_finite_prior_trial(
    ...
    *,
    ...
    """Run the canonical finite-prior attack on a convex Gaussian-output release.

    This is the preferred API for new code. The trial object owns D^-, support S,
    the hidden target position, source IDs, and prior weights.
    """
    d_minus = ArrayDataset(
        trial.D_minus_X,
        trial.D_minus_y,
        name="finite_prior_D_minus",
    )

    attack = run_convex_ball_output_finite_prior_attack(
        release,
        d_minus,
        prior_records=support_to_records(trial.support),
        prior_weights=trial.support.weights,
        known_label=(
            int(trial.support.center_y) if known_label is None else int(known_label)
        ),
...
def attack_nonconvex_finite_prior_trial(
    trace: DPSGDTrace,
    trial: FinitePriorTrial,
    *,
    ...
    """Run the canonical finite-prior attack on a residualized DP-SGD transcript.

    The same FinitePriorTrial object used for convex attacks is reused here.
    Only the observation model/scorer changes.
    """
    attack = run_ball_trace_finite_prior_attack(
        trace,
        support_to_records(trial.support),
        prior_weights=trial.support.weights,
        cfg=cfg,
