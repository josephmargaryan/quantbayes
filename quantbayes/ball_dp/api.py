...
def embed_torch_loader(
    *,
    ...
    """
    Embed a SINGLE torch DataLoader that yields (x,y).
    ...
    meta = {
        ...
        "Z_max_norm": float(np.max(l2_norms(Z))),
    }
    return Z, Y, meta


def embed_torch_loader_aug_pairs(
    *,
    loader,
    encoder,
    augment_fn: Callable,
    ...
    """
    Compute (e, e_aug) embedding pairs for augmentation-consistency replacement models.

    loader yields (x,y). We form x_aug = augment_fn(x) and embed both:
      e = phi(x), e_aug = phi(x_aug)

    Returns:
      Z:     (N,d) float32   embeddings of original
      Z_aug: (N,d) float32   embeddings of augmented
      y:     (N,)   int64
      meta:  dict

    DP note:
      Apply the SAME record-definition preprocessing (normalize/clip) to both Z and Z_aug.
      This lets you interpret augmentation pairs as candidate substitutions in the SAME record space.
    """
    import torch
    ...
    def _run():
        ...
        for x, y in loader:
            ...
            if isinstance(z, (tuple, list)):
                z = z[0]
            if isinstance(za, (tuple, list)):
                za = za[0]

            z = z.detach().float().cpu().numpy().astype(np.float32)
            za = za.detach().float().cpu().numpy().astype(np.float32)
            y_np = y.detach().cpu().numpy().astype(np.int64)

            z = maybe_l2_normalize(z, bool(l2_normalize))
            za = maybe_l2_normalize(za, bool(l2_normalize))
            if l2_clip_norm is not None:
                z = clip_l2_rows(z, float(l2_clip_norm))
                za = clip_l2_rows(za, float(l2_clip_norm))
    ...
    meta = {
        ...
        "l2_clip_norm": None if l2_clip_norm is None else float(l2_clip_norm),
        "Z_shape": tuple(Z.shape),
        "Z_aug_shape": tuple(Za.shape),
        "Z_max_norm": float(np.max(l2_norms(Z))),
        "Z_aug_max_norm": float(np.max(l2_norms(Za))),
    }
    return Z, Za, Y, meta
