...
def predict_eqx_svm(mdl, Xnp):
    Xj = jnp.asarray(Xnp.astype(np.float32))
    scores = jax.vmap(mdl)(Xj)
    return np.where(np.asarray(scores) >= 0.0, 1, -1).astype(np.int64)
...
if __name__ == "__main__":
    """
    Demo:
      (1) Baseline squared-hinge SVM on normalized embeddings
      (2) RFF(embeddings) + squared-hinge SVM
      (3) DP Gaussian output perturbation for both, using Ball-adjacency radius r_policy in embedding space.
          For RFF we use r_eff = Lpsi * r_policy with tight Lpsi = sqrt(2/m)*||Omega||_2 (SVD).
    """
    import numpy as np
    import jax
    import jax.numpy as jnp
    ...
    def svm_accuracy(model, X, y_pm1):
    ...
    def compute_Lpsi_svd(omega: jnp.ndarray, m_rff: int, safety: float = 1e-6) -> float:
        # Tight Lipschitz constant from theory: Lpsi = sqrt(2/m) * ||Omega||_2
    ...
    def dp_eval_from_sigma(
        ...
        for _ in range(num_trials):
            ...
            accs.append(svm_accuracy(mdl, X_eval, y_eval))
    ...
    # ERM regularization (must match objective)
    lam = 5e-2  # slightly larger than 1e-2 tends to behave better with conservative bounds

    # ==========================================================
    # (1) Baseline SVM on embeddings
    # ==========================================================
    print("\n=== Baseline: SVM on embeddings ===")
    model0 = SquaredHingeSVMEqx(d_in=d_embed, key=k_model0)
    state0 = None
    ...
    acc0 = svm_accuracy(model0_tr, X_val, y_val)
    print(f"Val acc (non-DP): {acc0:.4f}")

    # DP constants (baseline)
    Lz0 = float(lz_squared_hinge_bound(B=B_e, lam=lam, include_bias=True))
    ...
    # (2) RFF + SVM
    # ==========================================================
    print("\n=== RFF: SVM on Phi(embeddings) ===")

    m_rff = 2048
    gamma = 1.0
    W_clip = 5.0  # optional; controls frequency magnitude; safe to keep public
    ...
    model1 = SquaredHingeSVMEqx(d_in=m_rff, key=k_model1)
    state1 = None
    ...
    acc1 = svm_accuracy(model1_tr, Xr_val, yr_val)
    print(f"Val acc (non-DP): {acc1:.4f}")

    # DP constants (RFF)
    B_phi = float(rff_feature_norm_bound())  # sqrt(2)
    Lpsi = compute_Lpsi_svd(omega, m_rff, safety=1e-6)  # tight, certified
    r_eff = float(Lpsi) * float(r_policy)

    Lz1 = float(lz_squared_hinge_bound(B=B_phi, lam=lam, include_bias=True))
