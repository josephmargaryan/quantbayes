# quantbayes/ball_dp/heads/softmax_eqx.py
...
class SoftmaxLinearEqx(eqx.Module):
    W: jnp.ndarray  # (K, d)
    b: jnp.ndarray  # (K,)
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        return self.W @ x + self.b  # logits, (K,)


def softmax_objective(
    model: SoftmaxLinearEqx,
    state: Any,
    xb: jnp.ndarray,  # (B, d)
    yb: jnp.ndarray,  # (B,) integer labels in {0,...,K-1}
    key: jr.PRNGKey,
    *,
    lam: float,
) -> Tuple[jnp.ndarray, Dict[str, jnp.ndarray]]:
    logits = eqx.filter_vmap(model)(xb)  # (B, K)
    logZ = jax.nn.logsumexp(logits, axis=1)  # (B,)
...
if __name__ == "__main__":
    """
    Demo:
      (1) Baseline multiclass softmax regression on normalized embeddings
      (2) RFF(embeddings) + multiclass softmax regression
      (3) DP Gaussian output perturbation for both.
    ...
    def softmax_accuracy(model, X, y):
    ...
    def dp_eval_from_sigma(
        ...
        for _ in range(num_trials):
            ...
            accs.append(softmax_accuracy(mdl, X_eval, y_eval))
    ...
    X, y = make_blobs_multiclass(
        k_data, n_per_class=n_per_class, num_classes=K, d=d_embed
    )
    ...
    # With the conservative softmax Lz bound, too-small lam can explode sigma.
    # For demo stability, use lam >= 0.1.
    lam = 0.1

    # ==========================================================
    # (1) Baseline softmax on embeddings
    # ==========================================================
    print("\n=== Baseline: softmax on embeddings ===")
    model0 = SoftmaxLinearEqx(d_in=d_embed, n_classes=K, key=k_model0)
    state0 = None

    def loss_fn0(mdl, state, xb, yb, key):
        return softmax_objective(mdl, state, xb, yb, key, lam=lam)
    ...
    acc0 = softmax_accuracy(model0_tr, X_val, y_val)
    print(f"Val acc (non-DP): {acc0:.4f}")

    # DP constants (baseline)
    Lz0 = float(lz_softmax_linear_bound(B=B_e, lam=lam, include_bias=True))
    ...
    model1 = SoftmaxLinearEqx(d_in=m_rff, n_classes=K, key=k_model1)
    state1 = None
    ...
    acc1 = softmax_accuracy(model1_tr, Xr_val, yr_val)
    ...
    Lz1 = float(lz_softmax_linear_bound(B=B_phi, lam=lam, include_bias=True))
