...
DEFAULT_DATASET_CONFIG_NAME = "split"
...
def default_output_path(
    ...
    return default_cache_dir(output_root) / f"emotion_{model_slug}_embeddings.npz"


def load_or_create_emotion_text_embeddings(
    batch_size: int = 128,
    ...
    dataset_config_name: str | None = DEFAULT_DATASET_CONFIG_NAME,
):
    ...
    return load_or_create_text_classification_embeddings(
        ...
        train_desc="Emotion train",
        test_desc="Emotion test",
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute/cache canonical-split Emotion text embeddings and save JAX-ready arrays."
    )
    ...
    parser.add_argument(
        "--dataset-config-name", type=str, default=DEFAULT_DATASET_CONFIG_NAME
    )
...
def main() -> None:
    ...
    X_train, y_train, X_test, y_test = load_or_create_emotion_text_embeddings(
    ...
    print(f"dataset_config_name: {args.dataset_config_name}")
