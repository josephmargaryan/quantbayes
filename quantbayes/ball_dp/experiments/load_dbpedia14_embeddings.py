...
DEFAULT_DATASET_ID = "fancyzhx/dbpedia_14"
...
def batch_to_texts(batch: dict[str, list]) -> list[str]:
    ...
    return [
        join_nonempty_texts(title, content, sep=". ")
...
def default_output_path(
    ...
    return default_cache_dir(output_root) / f"dbpedia14_{model_slug}_embeddings.npz"


def load_or_create_dbpedia14_text_embeddings(
    batch_size: int = 128,
    ...
    return load_or_create_text_classification_embeddings(
        ...
        train_desc="DBpedia-14 train",
        test_desc="DBpedia-14 test",
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute/cache canonical-split DBpedia-14 text embeddings and save JAX-ready arrays."
    )
...
def main() -> None:
    ...
    X_train, y_train, X_test, y_test = load_or_create_dbpedia14_text_embeddings(
