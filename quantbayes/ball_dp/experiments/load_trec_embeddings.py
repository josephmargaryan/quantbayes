...
def load_or_create_trec_text_embeddings(
    batch_size: int = 128,
    ...
    if label_space not in {"coarse", "fine"}:
        raise ValueError(f"label_space must be 'coarse' or 'fine', got {label_space!r}")

    label_field = "coarse_label" if label_space == "coarse" else "fine_label"
    ...
    return load_or_create_text_classification_embeddings(
        ...
        train_desc=f"TREC-{label_space} train",
        test_desc=f"TREC-{label_space} test",
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute/cache canonical-split TREC text embeddings and save JAX-ready arrays."
    )
...
def main() -> None:
    ...
    X_train, y_train, X_test, y_test = load_or_create_trec_text_embeddings(
