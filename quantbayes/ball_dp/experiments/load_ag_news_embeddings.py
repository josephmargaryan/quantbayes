...
DEFAULT_DATASET_ID = "fancyzhx/ag_news"
...
def default_output_path(
    ...
    return default_cache_dir(output_root) / f"ag_news_{model_slug}_embeddings.npz"


def load_or_create_ag_news_text_embeddings(
    batch_size: int = 128,
    ...
    return load_or_create_text_classification_embeddings(
        ...
        train_desc="AG News train",
        test_desc="AG News test",
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute/cache canonical-split AG News text embeddings and save JAX-ready arrays."
    )
