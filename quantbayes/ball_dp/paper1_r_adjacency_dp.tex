...
On nine embedding benchmarks, matched-privacy experiments with the ridge-prototype model show that r-adjacency privacy can materially reduce the Gaussian noise scale, tighten reconstruction bounds, and often improve predictive accuracy relative to a standard global-replacement comparator. At a representative $q80$ radius and $\varepsilon=1$, the Ball mechanism reduces the noise scale by $1.42\times$ to $3.11\times$ and lowers the direct exact-identification upper bound against a same-noise standard comparator by $15.8\%$ to $54.2\%$ across the completed runs.
\end{abstract}
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item We evaluate the framework on nine embedding benchmarks. Across the completed ridge-prototype runs, the local mechanism consistently reduces Gaussian noise and tightens reconstruction bounds relative to a standard global comparator, and frequently improves classification accuracy at the same $(\varepsilon,\delta)$.
\end{itemize}
...
\begin{table*}[t]
\centering
\caption{Aggregate ridge-prototype comparison across datasets at the representative $q80$, $\varepsilon=1$ setting. The Ball mechanism consistently lowers the Gaussian noise scale at matched privacy. The bound columns report the direct exact-identification upper bound for Ball and for the same-noise standard comparator, i.e., the standard sensitivity evaluated at $\sigma_{\mathrm{Ball}}$. This same-noise comparison isolates the geometric effect of local sensitivity and is not a matched-privacy comparison. Under matched Gaussian calibration, the Ball and standard direct bounds are identical and are therefore not shown as separate bound columns.}
\label{tab:aggregate-summary}
\scriptsize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrrrrrrrrr}
\toprule
Dataset & Acc (Ball) & Acc (Std) & Acc gain & Bound (Ball) & Bound (Std @ $\sigma_{\mathrm{Ball}}$) & Bound red. \% & $\sigma$ (Ball) & $\sigma$ (Std) & $\sigma$ ratio \\
\midrule
AG News-embeddings            & 0.8345 & 0.8186 & 0.0159 & 0.0972 & 0.1155 & 15.78 & 0.0099 & 0.0141 & 1.4201 \\
BANKING77-embeddings          & 0.1234 & 0.0328 & 0.0906 & 0.0972 & 0.1241 & 21.61 & 0.1034 & 0.1656 & 1.6019 \\
CIFAR-10-embeddings           & 0.6977 & 0.4572 & 0.2405 & 0.0972 & 0.1673 & 41.87 & 0.0140 & 0.0337 & 2.4048 \\
DBpedia-14-embeddings         & 0.9353 & 0.9347 & 0.0007 & 0.0972 & 0.1164 & 16.47 & 0.0021 & 0.0030 & 1.4407 \\
Emotion-embeddings            & 0.2145 & 0.2075 & 0.0070 & 0.0972 & 0.1180 & 17.56 & 0.0708 & 0.1043 & 1.4740 \\
IMDb-embeddings               & 0.5668 & 0.5316 & 0.0351 & 0.0972 & 0.1225 & 20.65 & 0.0427 & 0.0671 & 1.5706 \\
MNIST-embeddings              & 0.8150 & 0.5490 & 0.2660 & 0.0972 & 0.2121 & 54.15 & 0.0090 & 0.0281 & 3.1051 \\
TREC-6-embeddings             & 0.1000 & 0.0700 & 0.0300 & 0.0972 & 0.1158 & 16.00 & 0.2096 & 0.2990 & 1.4267 \\
Yelp Review Full-embeddings   & 0.4293 & 0.4284 & 0.0008 & 0.0972 & 0.1204 & 19.21 & 0.0017 & 0.0026 & 1.5251 \\
\bottomrule
\end{tabular}
\end{table*}

Two patterns stand out. First, the exact ridge sensitivity formula translates almost linearly into lower released noise at matched $(\varepsilon,\delta)$, which in turn often improves accuracy by a visible margin. The effect is largest on datasets where the global same-label diameter is much larger than the chosen local policy radius. Second, the matched-privacy direct Gaussian bound is identical for the Ball and standard mechanisms, as expected from the ratio $\Delta_2/\sigma$ under Gaussian calibration. The reconstruction-geometry advantage appears in the same-noise comparison: when the standard global sensitivity is evaluated at $\sigma_{\mathrm{Ball}}$, its direct exact-identification bound is consistently worse than the Ball bound.
...
\begin{table}[t]
\centering
\caption{Placeholder for the aggregate finite-prior exact-identification attack results. Replace the \textbf{TBD} entries after running the Gaussian MAP attack sweep.}
\label{tab:attack-aggregate}
\footnotesize
\begin{tabular}{lccc}
\toprule
Dataset & Empirical $\bar p_{\mathrm{exact}}$ & Oblivious baseline $1/m$ & Advantage \\
\midrule
AG News-embeddings          & \todoresult & \todoresult & \todoresult \\
BANKING77-embeddings        & \todoresult & \todoresult & \todoresult \\
CIFAR-10-embeddings         & \todoresult & \todoresult & \todoresult \\
DBpedia-14-embeddings       & \todoresult & \todoresult & \todoresult \\
Emotion-embeddings          & \todoresult & \todoresult & \todoresult \\
IMDb-embeddings             & \todoresult & \todoresult & \todoresult \\
MNIST-embeddings            & \todoresult & \todoresult & \todoresult \\
TREC-6-embeddings           & \todoresult & \todoresult & \todoresult \\
Yelp Review Full-embeddings & \todoresult & \todoresult & \todoresult \\
\bottomrule
\end{tabular}
\end{table}
