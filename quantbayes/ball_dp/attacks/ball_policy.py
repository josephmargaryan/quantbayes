...
from .gradient_based import (
    DPSGDTrace,
    _clip_per_example_grad_tree,
    _clip_single_grad,
    _per_candidate_grads,
    _single_example_grad,
    _step_mean_denominator,
    _step_target_batch_size,
    _tree_batch_dot,
    _tree_batch_l2_norms,
    _tree_l2_sq,
    _tree_scalar_mul,
    _tree_sub,
...
def _tree_batch_scalar_mul(tree: Any, scalar: float | jnp.ndarray) -> Any:
    scalar = jnp.asarray(scalar)

    return jax.tree_util.tree_map(
        lambda x: x * scalar.astype(jnp.asarray(x).dtype),
        tree,
    )


def _finite_prior_log_scores_vectorized(
    trace: DPSGDTrace,
    ...
    *,
    selected_steps: Sequence[Any],
    mode: str,
    q_by_step: Optional[Sequence[float]],
    target_index: Optional[int],
    loss_fn: ExampleLossFn,
    seed: int,
) -> np.ndarray:
    """Vectorized finite-prior log posterior scores.

    This evaluates the same objective as the candidate-by-candidate finite-prior
    Bayes attack, but computes all candidate gradients for each retained step in
    one JAX vmap call. The returned scores are log posterior values up to a
    ...
    X = jnp.asarray(
        np.stack(
            [np.asarray(rec.features, dtype=np.float32) for rec in records],
            axis=0,
        ),
        dtype=jnp.float32,
    )
    y = jnp.asarray([int(rec.label) for rec in records], dtype=jnp.int32)

    totals = jnp.asarray(
        -np.log(np.asarray(probs, dtype=np.float64)),
        dtype=jnp.float32,
    )
    ...
    for idx, step in enumerate(selected_steps):
        ...
        grad_key = jr.PRNGKey(int(seed + 10007 * int(step.step)))

        cand = _per_candidate_grads(
            step.model_before,
            trace.state,
            loss_fn,
            X,
        ...
        cand, _ = _clip_per_example_grad_tree(cand, float(step.clip_norm))

        reduction = str(trace.reduction).lower()

        if reduction == "mean":
            cand = _tree_batch_scalar_mul(cand, 1.0 / _step_mean_denominator(step))
        elif reduction != "sum":
            raise ValueError("trace.reduction must be one of {'mean', 'sum'}.")

        cand_norm_sq = _tree_batch_l2_norms(cand) ** 2
        obs_norm_sq = _tree_l2_sq(obs)
        dot = _tree_batch_dot(cand, obs)
        sq1 = obs_norm_sq - 2.0 * dot + cand_norm_sq
        ...
        if mode == "known_inclusion":
            ...
            if target_index is not None:
                # selected_steps has already applied step_mode; this branch preserves
                # the exact objective for step_mode='all'.
                include = _step_contains_target(step, int(target_index))
            ...
            if sigma > 0.0:
                totals = totals + sq1 / (2.0 * sigma * sigma)
            else:
                totals = totals + sq1

        elif mode == "unknown_inclusion":
            if sigma <= 0.0:
                raise ValueError(
            ...
            if q_by_step is None:
                raise ValueError("q_by_step is required for unknown-inclusion scoring.")

            q = float(q_by_step[idx])
            sq0 = obs_norm_sq

            if q <= 0.0:
                totals = totals + sq0 / (2.0 * sigma * sigma)
            elif q >= 1.0:
                totals = totals + sq1 / (2.0 * sigma * sigma)
            else:
                q_j = jnp.asarray(q, dtype=jnp.float32)
                a0 = jnp.log1p(-q_j) - sq0 / (2.0 * sigma * sigma)
                a1 = jnp.log(q_j) - sq1 / (2.0 * sigma * sigma)
                totals = totals - jnp.logaddexp(a0, a1)

        else:
            raise ValueError(
                "mode must be one of {'known_inclusion', 'unknown_inclusion'}."
            )
...
def run_ball_trace_finite_prior_attack(
    ...
    *,
    ...
    log_scores_arr = _finite_prior_log_scores_vectorized(
        trace,
        filtered_records,
        probs,
        selected_steps=selected_steps,
        mode=mode,
        q_by_step=q_by_step,
        target_index=(None if effective_step_mode == "present_steps" else target_index),
        loss_fn=resolved_loss_fn,
        seed=int(cfg.seed),
    )
    ...
    diagnostics: Dict[str, Any] = {
        "mode": str(mode),
        "algorithm": "finite_prior_exact_bayes_vectorized",
