...
from quantbayes.ball_dp.reconstruction.vision.datasets import load_mnist_numpy
...
def main():
    ...
    ap.add_argument(
        "--n_shadow_per_class", type=int, default=200
    )  # used only as oracle pool side-info
    ap.add_argument("--n_target_per_class", type=int, default=30)

    ap.add_argument("--max_targets", type=int, default=200)  # cap for speed
    ...
    # Public bound for pixels in [0,1]^784
    B_public = pixel_l2_bound_unit_box(784, 1.0)
    ...
    # Radius policy computed on D^- only (fixed)
    ...
    # Lz bound for softmax linear head
    Lz = float(
    ...
    # Fit θ* once per target; cache (theta_hat, unflatten)
    print(
        f"[MNIST] Fitting θ* for {X_tg.shape[0]} targets... (this is the expensive part)"
    )
    ...
    for i in range(X_tg.shape[0]):
        ...
        if (i + 1) % 25 == 0 or (i + 1) == X_tg.shape[0]:
            print(f"  fitted {i+1}/{X_tg.shape[0]}")

    # Evaluate DP recon errors for each (r, eps)
    curves_mse: Dict[str, Tuple[List[float], List[float]]] = {}
    curves_l2: Dict[str, Tuple[List[float], List[float]]] = {}

    # Oracle pool (D^- plus optional side knowledge)
    oracle_pool = np.concatenate([X_fix, X_sh], axis=0) if X_sh.size else X_fix
    ...
    save_errorbar_plot(
        x=eps_list,
        curves=curves_mse,
        title="MNIST pixel-space convex softmax recon: MSE vs epsilon",
        xlabel="epsilon",
        ylabel="MSE(x_hat, x)",
        out_path=out_dir / "mnist_convex_softmax_mse_vs_eps.png",
        xscale_log=True,
    )
    save_errorbar_plot(
        x=eps_list,
        curves=curves_l2,
        title="MNIST pixel-space convex softmax recon: L2 vs epsilon",
        xlabel="epsilon",
        ylabel="||x_hat - x||_2",
        out_path=out_dir / "mnist_convex_softmax_l2_vs_eps.png",
        xscale_log=True,
    )
    ...
    write_json(out_dir / "mnist_convex_softmax_report.json", report)

    # Save example recon grid for one (eps, percentile) choice
    if args.save_examples:
        p = float(args.examples_percentile)
        r_ex = r_ball[p]
        eps_ex = float(args.examples_eps)
        print(f"[MNIST] Saving example recon grid at p={p} r={r_ex:.4f} eps={eps_ex:g}")

        # reconstruct first N examples
        Nshow = min(int(args.examples_n), X_tg.shape[0])
        orig_imgs = unflatten_mnist(X_tg[:Nshow])
        recon_flat = []
        ...
        save_recon_grid(
            orig_imgs,
            recon_imgs,
            out_dir / f"mnist_examples_softmax_eps{eps_ex:g}_p{int(p)}.png",
            n=Nshow,
            title=f"MNIST recon (softmax) eps={eps_ex:g} p{int(p)}",
        )

    print(f"[OK] MNIST outputs in: {out_dir}")
