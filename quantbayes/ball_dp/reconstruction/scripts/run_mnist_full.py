...
def _prepare_data_embed(args) -> Dict[str, object]:
    """
    Public encoder φ: ResNet18(ImageNet) to 512-d embeddings.
    Also returns target images in original MNIST resolution (1x28x28).
    """
    ...
    _print("[MNIST/embed] Extracting embeddings (this may take a bit)...")
    ...
    K = int(np.max(y) + 1)
    # If we L2-normalize, we can treat B=1 as public bound.
    B_public = (
        1.0 if args.embed_l2_normalize else float(np.max(np.linalg.norm(E, axis=1)))
    )

    _print(
        f"[MNIST/embed] got E={E.shape}, X_img={X_img.shape}, y={y.shape}, B_public≈{B_public:.3f}"
    )

    return {
        "mode": "embed",
        "X": E,  # treat embeddings as records
        "X_img": X_img,  # true images (N,1,28,28) in [0,1]
        "y": y,
        "K": K,
...
def _maybe_train_decoder(
    ...
    """
    Train a public decoder g(e)->x_flat on (E, X_img).
    This is post-processing only; does not affect DP.
    """
...
def main():
    ...
    ap.add_argument("--embed_max_batches", type=int, default=0, help="0 means no limit")
    ...
    if args.head == "softmax":
        ...
        _print(f"[MNIST] Fitting θ* for {X_tg.shape[0]} targets... (expensive)")
        ...
        # prototypes: cache mus/counts per target quickly using sums/counts
        _print(
    ...
    # optional image curves if embed+decoder
    curves_img_mse: Dict[str, Tuple[List[float], List[float]]] = {}

    def evaluate_setting_softmax(
        r: float, eps: float
    ...
    def evaluate_setting_prototypes(
        r: float, eps: float
    ) -> Tuple[np.ndarray, np.ndarray, Optional[np.ndarray]]:
        errs_l2, errs_mse = [], []
        errs_img = [] if (decoder is not None) else None

        # fixed sums/counts for attacker
    ...
    eval_fn = (
        evaluate_setting_softmax
        if args.head == "softmax"
        else evaluate_setting_prototypes
    )

    _print("[MNIST] Evaluating DP recon curves...")
    ...
    save_errorbar_plot(
        x=eps_list,
        curves=curves_l2,
        title=f"MNIST {args.mode} {args.head}: ||x̂-x||₂ vs ε",
        xlabel="epsilon",
        ylabel="L2 error",
    ...
    save_errorbar_plot(
        x=eps_list,
        curves=curves_mse,
        title=f"MNIST {args.mode} {args.head}: MSE(x̂,x) vs ε",
        xlabel="epsilon",
        ylabel="MSE",
    ...
    if decoder is not None:
        save_errorbar_plot(
            ...
            title=f"MNIST embed+decoder {args.head}: MSE(decoded(x̂), img) vs ε",
            xlabel="epsilon",
    ...
    if args.save_examples:
        ...
        _print(f"[MNIST] Saving example grid: p{int(p)} eps={eps_ex:g} N={Nshow}")

        if args.mode == "pixel":
            orig_imgs = X_img_tg[:Nshow]
            recon_flat = []

            # reconstruct with the chosen r,eps
            if args.head == "softmax":
                Delta = float(
                    erm_sensitivity_l2(
            ...
            recon_imgs = clip01(unflatten_mnist(recon_flat))
            save_recon_grid(
                orig=orig_imgs,
                recon=recon_imgs,
                out_path=out_dir
            ...
            # embed mode: decode recon embeddings if decoder available
            if decoder is None:
                _print(
                    "[MNIST/embed] No decoder trained; set --train_decoder to save image grids."
                )
            else:
                orig_imgs = X_img_tg[:Nshow]
                recon_imgs_list = []

                # Build recon embeddings, then decode
                if args.head == "softmax":
                    Delta = float(
                        erm_sensitivity_l2(
                            lz=Lz, r=float(r_ex), lam=float(args.lam), n=n_full
                ...
                save_recon_grid(
                    ...
                    recon=recon_imgs,
                    out_path=out_dir
                    / f"mnist_embed_{args.head}_decoded_examples_eps{eps_ex:g}_p{int(p)}.png",
                    n=Nshow,
                    title=f"MNIST embed+decoder {args.head} eps={eps_ex:g} p{int(p)}",
                )
