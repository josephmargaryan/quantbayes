# quantbayes/ball_dp/privacy/ball_dpsgd_train.py
...
from .ball_dpsgd import BallDPPrivacyEngine, BallDPSGDConfig
...
def ball_dp_eqx_train(
    ...
    y_val: Array,
    *,
    ball_dp_config: BallDPSGDConfig,
    batch_size: int,
    num_epochs: int,
    patience: int,
    key: jax.Array,
    ...
    """
    Ball-DP-SGD training loop with WOR-RDP accountant.
    Fixed-size minibatches (shuffle) with sample_rate q = batch_size / N.
    """
    engine = BallDPPrivacyEngine(ball_dp_config)
    ...
    for epoch in range(1, int(num_epochs) + 1):
        ...
        # print epsilon progress
        try:
            eps = engine.epsilon()
            print(
                f"[Ball-DP] epoch {epoch:03d} | train={train_hist[-1]:.4f} | val={v:.4f} | eps≈{eps:.3f} (δ={ball_dp_config.delta:g})"
            )
        except Exception:
            print(
                f"[Ball-DP] epoch {epoch:03d} | train={train_hist[-1]:.4f} | val={v:.4f}"
            )
