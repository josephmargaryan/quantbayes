...
class SegmentationModel(BaseModel):
    ...
    def train_step(self, model, state, X, Y, key):
        ...
        loss, grads = jax.value_and_grad(loss_func)(model)
    ...
    def predict_step(self, model, state, X, key):
        # Return a thresholded mask (0/1) for each image in the batch.
        preds = jax.vmap(model)(X)
        return (preds > 0.5).astype(jnp.int32)

    def visualize(self, X_test, Y_test, Y_pred, title="Segmentation Results"):
        """
        Visualize segmentation results for one sample.
    ...
    def fit(self, model, X_train, Y_train, X_val, Y_val, num_epochs=100, lr=1e-3, patience=10):
        ...
        Train the segmentation model.
          X_train: (batch, H, W, C)
          Y_train: (batch, H, W) (binary masks)
        """
