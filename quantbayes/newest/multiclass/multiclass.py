...
class MulticlassClassificationModel(BaseModel):
    ...
    def cross_entropy_loss(model, x, y):
        # y is assumed to be integer labels.
    ...
    def visualize(self, X_test, y_test, y_pred, feature_indices: tuple = (0, 1), title="Decision Boundary"):
        ...
        # Instead of using predict_step with double vmapping, use a lambda that directly calls the model.
        preds_grid = jax.vmap(lambda x: jnp.argmax(self.model(x)))(X_grid)
