...
class BinaryClassificationModel(BaseModel):
    ...
    def binary_cross_entropy_loss(model, x, y):
        # Compute predictions over the batch.
    ...
    def predict_step(self, model, state, X, key):
        # Get predicted probabilities and convert to class labels using 0.5 threshold.
        preds = jax.vmap(model)(X)
    ...
    def visualize(self, X_test, y_test, y_pred, feature_indices: tuple = (0, 1), title="Decision Boundary"):
        ...
        # Create a grid where non-visualized features are fixed to their mean.
        X_grid = []
        ...
        # Use vmap with a JAX-compatible lambda using jnp.where.
        preds_grid = jax.vmap(
            lambda x: jnp.where(jax.nn.sigmoid(self.model.linear(x)) > 0.5, 1, 0)
        )(X_grid)
