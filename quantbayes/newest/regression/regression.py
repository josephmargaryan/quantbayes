...
class RegressionModel(BaseModel):
    ...
    def train_step(self, model, state, X, y, key):
        """
        Performs a single training step: computes gradients, updates model parameters,
        and returns the loss, updated model, and updated optimizer state.
        """
        # Define a loss function that only depends on the model parameters.
        def loss_fn(m):
            return RegressionModel.mse_loss(m, X, y)
        
        # Compute the loss and gradients.
        loss, grads = jax.value_and_grad(loss_fn)(model)
        # Use the stored optimizer (set in fit) to compute parameter updates.
    ...
    def predict_step(self, model, state, X, key):
        """
        Runs inference by vectorizing the model over the input.
        """
    ...
    def fit(self, model, X_train, y_train, X_val, y_val, num_epochs=100, lr=1e-3, patience=10):
        ...
        for epoch in range(num_epochs):
            ...
            # If self.batch_size is specified, you could iterate over mini-batches
