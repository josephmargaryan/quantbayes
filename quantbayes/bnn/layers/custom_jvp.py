...
@jax.custom_jvp
def fft_conv2d_custom(kernel: jnp.ndarray, x: jnp.ndarray) -> jnp.ndarray:
    """
    2-D circular convolution via FFT.
    kernel: (H_pad, W_pad) real
    x:      (..., H_in, W_in) real
    returns (..., H_pad, W_pad) real
    """
...
@fft_conv2d_custom.defjvp
def fft_conv2d_custom_jvp(primals, tangents):
    kernel, x = primals
    dkernel, dx = tangents
    ...
    dKf = jnp.fft.fftn(dkernel, axes=(0, 1)) if dkernel is not None else 0.0
...
class Circulant2d:
    """
    2-D circulant convolution layer with custom JVP.
    """
    ...
    def __init__(
        ...
        name: str = "circulant2d",
        kernel_prior_fn=lambda shape: dist.Normal(0.0, 1.0).expand(shape).to_event(2),
        bias_prior_fn=lambda shape: dist.Normal(0.0, 1.0).expand(shape).to_event(2),
    ):
        self.H_in = H_in
        self.W_in = W_in
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        kernel = numpyro.sample(
            f"{self.name}_kernel",
            self.kernel_prior_fn((self.H_pad, self.W_pad)),
        )
        bias = numpyro.sample(
            f"{self.name}_bias",
            self.bias_prior_fn((self.H_pad, self.W_pad)),
        )
        out = fft_conv2d_custom(kernel, x)
        return out + bias
...
class AdaptiveSpectralCirculantLayer(SpectralCirculantLayer):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        raw_alpha = self.alpha_global + delta_alpha

        alpha_vec = 0.1 + jax.nn.softplus(raw_alpha - self._m)
...
class AdaptiveSpectralCirculantLayer2d(SpectralCirculantLayer2d):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        raw_full = self.alpha_global + delta_alpha_full

        alpha_full = 0.1 + jax.nn.softplus(raw_full - self._m)
