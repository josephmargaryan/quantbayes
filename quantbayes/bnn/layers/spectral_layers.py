...
__all__ = [
    ...
    "SpectralTokenMixer",
    # Graph Chebyshev Sobolev filter
    "GraphChebSobolev",
]
...
# ------------------------- Graph Chebyshev Sobolev ---------------------------


def _power_iter_sym(
    matvec: Callable[[jnp.ndarray], jnp.ndarray], dim: int, n_iter: int = 50
) -> float:
    ...
    lam = jnp.dot(v, matvec(v))
    return float(jnp.maximum(lam, 0.0))
...
class GraphChebSobolev:
    """
    Chebyshev spectral graph filter with Sobolev prior on polynomial degree.
    Y = sum_{k=0..K} T_k(L̃) X @ W_k + bias
      - L̃ = (2/λ_max) L - I ensures spectrum in [-1,1] without explicit eigendecomp.
      - Prior std_k ∝ 1/sqrt(1 + (k/K)^alpha)
    Shapes:
      X: (N, Fin) or (B, N, Fin)
      W_k: (Fin, Fout)
    ...
    def __call__(self, X: jnp.ndarray) -> jnp.ndarray:
        ...
        # per-degree prior std
        k = jnp.arange(self.K + 1, dtype=self.dtype)
        k_norm = jnp.where(self.K == 0, 0.0, k / self.K)
        std_k = self.init_scale / jnp.sqrt(1.0 + k_norm**alpha)  # (K+1,)

        # sample weights W_k and optional bias
        W = numpyro.sample(
            f"{self.name}_W",
            self.prior_fn(
                std_k[:, None, None]
                * jnp.ones((self.K + 1, self.Fin, self.Fout), self.dtype)
            ).to_event(3),
        )  # (K+1, Fin, Fout)
        ...
        if self.use_bias:
            b = numpyro.sample(
                f"{self.name}_b",
                dist.Normal(0.0, self.init_scale).expand([self.Fout]).to_event(1),
            )  # (Fout,)
