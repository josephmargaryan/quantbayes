...
from .utils_ensemble import align_proba_to_global
...
class GreedyWeightedEnsembleSelector(BaseEstimator):
    ...
    def _get_oof(
        ...
        n = X.shape[0]
        classes = None if self.task == "regression" else np.unique(y)
        ...
        for tr, va in splitter.split(X, y):
            ...
            if self.task == "regression":
                oof[va] = m.predict(X[va]).ravel()
            else:
                P = m.predict_proba(X[va])
                # align to global classes
                cols = np.zeros((len(va), len(classes)), dtype=float)
    ...
    def predict(self, X: Union[np.ndarray, List]) -> np.ndarray:
        ...
        # classification – align to global class order
        mats = []
        for m in self.fitted_estimators_.values():
            P = m.predict_proba(X_arr)
            mats.append(align_proba_to_global(P, m.classes_, self.classes_))
