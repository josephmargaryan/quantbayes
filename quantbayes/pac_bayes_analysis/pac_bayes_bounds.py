...
def pac_bayes_kl_bound(empirical_loss, posterior, prior, n, delta):
    ...
    res = minimize_scalar(lambda p: abs(objective(p)), bounds=(lower, upper), method='bounded')
...
def pac_bayes_empirical_bernstein_bound(empirical_loss, empirical_var, posterior, prior, n, delta, c1=1.1, c2=1.1):
    """
    ...
    add_term = (1+c2) * sqrt(empirical_var * (kl_p + log(nu1/delta)) / (2*(n-1))) + (2*c2*(kl_p + log(nu1/delta)))/(n-1)
...
def example_synthetic():
    ...
    empirical_variances = np.array([0.05*0.95, 0.10*0.90, 0.15*0.85, 0.08*0.92, 0.12*0.88])
    ...
    eb_bound = pac_bayes_empirical_bernstein_bound(weighted_empirical_loss, 
                                                   weighted_empirical_var, 
                                                   posterior, prior, n, delta,
                                                   c1=1.1, c2=1.1)
