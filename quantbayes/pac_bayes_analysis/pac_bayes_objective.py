...
class BayesianLinear(nn.Module):
    ...
    def kl_divergence(self):
        ...
        kl_weight = 0.5 * (((weight_var + self.weight_mu**2) / self.prior_var) -
                           1 - self.weight_logvar + math.log(self.prior_var)).sum()
        bias_var = torch.exp(self.bias_logvar)
        kl_bias = 0.5 * (((bias_var + self.bias_mu**2) / self.prior_var) -
                         1 - self.bias_logvar + math.log(self.prior_var)).sum()
...
##############################################
def train_bayesian_net_bernstein(model, train_loader, optimizer, lam=1e-3, delta=0.05,
                                 c=7/3, num_epochs=10, device='cpu'):
    """
    ...
    for epoch in range(num_epochs):
        ...
        for data, target in train_loader:
            ...
            #                           + c*(KL + log(1/delta))/n
            n = data.size(0)
            extra_term = math.sqrt((2 * variance.item() * (kl.item() + math.log(1/delta))) / n) \
...
if __name__ == '__main__':
    ...
    train_bayesian_net_bernstein(model, train_loader, optimizer, lam=lam, delta=delta,
