...
class EncoderLayer(nn.Module):
    ...
    def __init__(
        self, self_attention, d_model, d_ff=None, dropout=0.1, activation="relu"
    ):
...
class PositionalEmbedding(nn.Module):
    ...
    def __init__(self, d_model, max_len=5000):
        ...
        div_term = torch.exp(
            -(math.log(10000.0) / d_model) * torch.arange(0, d_model, 2)
        )
    ...
    def __init__(
        ...
        if distil and e_layers > 1:
            ...
            self.conv_layers = nn.ModuleList(
                [ConvLayer(d_model, dropout=dropout) for _ in range(e_layers - 1)]
            )
