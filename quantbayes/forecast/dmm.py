...
class TransitionModel(nn.Module):
    """
    p(z_t | z_{t-1}): outputs mean and log-variance of z_t given z_{t-1}.
    """
    def __init__(self, z_dim=1, hidden_dim=16):
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim, 2 * z_dim)  # output: [mean, log_var] of z_t
        )

    def forward(self, z_prev):
        """Compute mean and log-variance for z_t given z_{t-1}."""
        out = self.net(z_prev)
...
class DeepMarkovModel(nn.Module):
    def __init__(self, x_dim=1, z_dim=1, hidden_dim=16):
        ...
        self.emission_model   = EmissionModel(z_dim=z_dim, hidden_dim=hidden_dim, x_dim=x_dim)
        self.inference_model = InferenceModel(z_dim=z_dim, x_dim=x_dim, hidden_dim=hidden_dim)
