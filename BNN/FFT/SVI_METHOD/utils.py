...
def train(X_train, y_train, bnn_model, hidden_dim=10, num_steps=1000, num_classes=None, track_loss=False):
    """
    ...
    init_args = {"hidden_dim": hidden_dim}
    ...
    for step in range(num_steps):
        update_args = {"hidden_dim": hidden_dim}
        if num_classes is not None and "num_classes" in bnn_model.__code__.co_varnames:
...
def inference(svi, params, X_test, hidden_dim=10, sample_from="y", num_classes=None):
    """
...
def visualize_regression(X_test, y_test, mean_predictions, lower_bound, upper_bound):
    """
    ...
    sorted_indices = jnp.argsort(X_test[:, 0])  # Sort by the first feature
    X_test_sorted = X_test[sorted_indices, 0]  # First feature for visualization
    ...
    plt.fill_between(
        X_test_sorted, lower_bound_sorted, upper_bound_sorted, color="gray", alpha=0.3, label="95% Confidence Interval"
    )
    plt.plot(X_test_sorted, mean_predictions_sorted, color="blue", label="Predicted Mean")
    plt.scatter(X_test_sorted, y_test_sorted, color="red", label="True Values", alpha=0.8, s=20)
...
def visualize_binary(
    X, y, mean_predictions, uncertainty, svi, params, features=(0, 1), resolution=100
):
    """
    ...
    xx, yy = jnp.meshgrid(jnp.linspace(x_min, x_max, resolution), jnp.linspace(y_min, y_max, resolution))
    ...
    grid_predictions = inference(svi, params, grid_points_full, hidden_dim=10, sample_from="y")
    ...
    plt.contourf(xx, yy, grid_mean_predictions, levels=100, cmap=plt.cm.RdYlBu, alpha=0.8)
    ...
    plt.scatter(X_selected[:, 0], X_selected[:, 1], c=y, edgecolor="k", cmap=plt.cm.RdYlBu, s=20)
...
def visualize_multiclass(
    X, y, mean_predictions, uncertainty, svi, params, num_classes, features=(0, 1), resolution=100
):
    """
    ...
    # Extend grid_points to match full feature dimensions
    grid_points_full = jnp.zeros((grid_points.shape[0], X.shape[1]))
    grid_points_full = grid_points_full.at[:, feature_1].set(grid_points[:, 0])
    grid_points_full = grid_points_full.at[:, feature_2].set(grid_points[:, 1])

    # Generate predictions for grid points
    grid_predictions = inference(svi, params, grid_points_full, sample_from="logits", num_classes=num_classes)
    grid_mean_predictions = jax.nn.softmax(grid_predictions.mean(axis=0), axis=-1)  # Convert logits to probabilities
    grid_uncertainty = -jnp.sum(grid_mean_predictions * jnp.log(grid_mean_predictions + 1e-9), axis=1)  # Entropy
    grid_classes = jnp.argmax(grid_mean_predictions, axis=1).reshape(xx.shape)
    grid_uncertainty = grid_uncertainty.reshape(xx.shape)

    # Plot decision boundaries
    plt.figure(figsize=(12, 8))
    plt.contourf(xx, yy, grid_classes, levels=num_classes, cmap=plt.cm.RdYlBu, alpha=0.6)
