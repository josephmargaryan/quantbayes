...
def hierarchical_regressor(X, y=None):
    ...
    weights_dense = numpyro.sample(
        "weights_dense",
        dist.Normal(0, 1 / jnp.sqrt(prec)).expand([num_particles, input_size, hidden_size]),
    )
    bias_dense = numpyro.sample(
        "bias_dense", dist.Normal(0, 1 / jnp.sqrt(prec)).expand([num_particles, hidden_size])
    )
    ...
    weights_out = numpyro.sample(
        "weights_out", dist.Normal(0, 1 / jnp.sqrt(prec)).expand([num_particles, hidden_size, 1])
    )
...
def hierarchical_multiclass(X, y=None, num_classes=3):
    ...
    weights_dense = numpyro.sample(
        "weights_dense",
        dist.Normal(0, 1 / jnp.sqrt(prec)).expand([num_particles, input_size, input_size]),
    )
    bias_dense = numpyro.sample(
        "bias_dense", dist.Normal(0, 1 / jnp.sqrt(prec)).expand([num_particles, input_size])
    )
    ...
    weights_out = numpyro.sample(
        "weights_out",
        dist.Normal(0, 1 / jnp.sqrt(prec)).expand([num_particles, input_size, num_classes]),
    )
    bias_out = numpyro.sample(
