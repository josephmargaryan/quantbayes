from BNN2.core.base_task import BaseTask
from BNN2.core.base_inference import BaseInference
from BNN2.utils.fft_module import fft_matmul
...
class FFTMultiClassSteinVI(BaseTask, BaseInference):
    """
    Bayesian Multiclass Classification Model with FFT-based Circulant Matrix Layer using Stein Variational Inference.
    """
    ...
    def multiclass_model(self, X, y=None):
        """
        Shallow Bayesian Multiclass Classification Model with FFT-based Circulant Matrix Layer.
        """
    ...
    def deep_multiclass_model(self, X, y=None):
        """
        Deep Bayesian Multiclass Classification Model with FFT-based Circulant Matrix Layer.
        """
        num_classes = self.num_classes
        input_size = X.shape[1]
        num_particles = self.num_particles
        ...
        weights_out = numpyro.sample(
            "weights_out",
            dist.Normal(0, 1).expand([num_particles, hidden2.shape[2], num_classes]),
        )
        bias_out = numpyro.sample(
            "bias_out", dist.Normal(0, 1).expand([num_particles, num_classes])
        )
...
if __name__ == "__main__":
    ...
    ############# Example Usage #############
    from BNN2.modules.fft.multiclass_classification.stein_vi import FFTMultiClassSteinVI
    from BNN2.utils.entropy_analysis import EntropyAndMutualInformation
    from BNN2.utils.generalization_bound import BayesianAnalysis
    ...
    key = jr.key(0)
    classifier = FFTMultiClassSteinVI(
        num_classes=3, num_particles=5, num_steps=100, model_type="deep"
    )
