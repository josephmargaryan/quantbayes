...
for page, text_blocks in font_issues.items():
    if text_blocks:
        ...
        for block in text_blocks:
            print(
                f"  - Text: '{block['text']}' (Font: {block['font']}) at {block['bbox']}"
            )
    else:
...
def extract_fonts(pdf_path):
    ...
    for font_name, details in fonts.items():
        ...
        print(
            f"PDF Font Name: {font_name} - Actual Font: {details['real_name']} - {status}"
        )
...
def extract_fonts_clean(pdf_path):
    ...
    for page_num in range(len(doc)):
        ...
        for font in font_list:
            ...
            if font_info:
                font_base_name = (
                    font_info[1] if len(font_info) > 1 else "Unknown"
                )  # Font name
...
extract_fonts_clean(
    "Joseph - Font Test - MC-US-10336-External-Flu- RSV RUO deck (1).pdf"
)
